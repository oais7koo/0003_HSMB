...
**Area-scan camera configuration.** The field experiments described here were conducted with an 8K area-scan camera equipped with a global shutter (50 μs integration), an autofocus 24–85 mm zoom lens, and a 200 W LED strobe synchronised to the integration window. This configuration preserves the two-dimensional simultaneous-exposure blur mechanism used in Section III and eliminates the directional-scan artefact characteristic of line-scan imaging, ensuring that the blur formation mechanism is consistent with that of the laboratory HSMB dataset.

**Factorial experimental design.** A three-factor factorial design was employed to quantify the combined effect of vehicle speed, camera-to-wall distance, and sensor gain on field motion blur. Three independent variables were controlled:
...
**Statistical power.** For condition-level correlation analysis (N = 50), an a-priori power calculation shows that a Pearson correlation of ρ = 0.5 is detectable with a statistical power of ≥ 0.95 at α = 0.05. This factorial protocol therefore provides substantially more statistical resolution than a narrow speed-only comparison and supports inferentially defensible conclusions about field-level performance.
...
The field protocol described in this section addresses two methodological requirements for an operationally meaningful validation of the HSMB metric. First, the adoption of an 8K area-scan camera aligns the field imaging mechanism with the laboratory HSMB dataset; both share two-dimensional simultaneous-exposure blur, removing the confound introduced by directional line-scan artefacts. Second, the factorial design (50 conditions) provides a correlation-analysis sample size that yields an a-priori statistical power of ≥ 0.95 for detecting moderate-to-strong correlations (ρ ≥ 0.5) at the 5 % significance level. Combined with permutation tests and Bootstrap confidence intervals, this design supports inferentially defensible conclusions. Together with the ISO-robustness sub-analysis, these features demonstrate that the HSMB metric maintains stability across photon-budget conditions typical of road-tunnel inspection and is suitable for real-time image-quality filtering in operational MTSS platforms.
...
> **한글 버전**: [ko/06_field_validation.md](ko/06_field_validation.md)
