...
흐름:
    ...
    Phase 2: oosync run --push-only 실행
    Phase 3: 결과 요약 출력
...
SYNC_SCRIPT = SKILLS_DIR / "oosync" / "scripts" / "oosync_run.py"
...
def run_phase2(dry_run: bool = False) -> bool:
    """Phase 2: oosync run --push-only. 성공 여부 반환."""
    mode_str = "(dry-run 미리보기)" if dry_run else "(실제 동기화)"
    print_phase(2, f"oosync run --push-only {mode_str}")
...
def run_pipeline(dry_run: bool = False, force: bool = False, skip_standard: bool = False) -> int:
    ...
    print(f"# oosync pipeline - 표준 검증 → 배포\n")
