...
def cmd_repair(project_name: str | None = None, dry_run: bool = False):
    ...
    """
    print("# oosync repair - file_hashes 재구성\n")
    if dry_run:
    ...
    if not dry_run:
        print(f"- 다음 `oosync view` / `oosync run` 부터 정확한 분류가 적용됩니다.")
    return 0
...
def main():
    ...
    if subcommand == "status":
        ...
        # 라우팅 규칙:
        # - `oosync run` (인자 없음) → 모든 Full 프로젝트에 일괄 push (push-only 기본값)
        # - `oosync run --push-only [...]` → 명시적 일괄 push (옵션 통과)
        # - `oosync run <project>` → 특정 프로젝트 비교/동기화
