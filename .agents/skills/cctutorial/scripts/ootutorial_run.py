...
def build_faq_section(skill_name: str, sections: dict[str, str], body: str) -> str:
    ...
    if not faqs:
        faqs.append("> 실전 사용 중 FAQ가 축적되면 이 섹션에 추가됩니다.\n>\n> `cctutorial add-faq {skill_name} \"질문\" \"답변\"` 으로 추가 가능")
...
def generate_skill_tutorial(skill_name: str) -> str:
    ...
    if template:
        ...
        tutorial += body
        tutorial += f"\n\n---\n\n> 생성일: {now} | cctutorial {OOTUTORIAL_VERSION}\n"
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ("help", "-h", "--help"):
        _print_skill_help("cctutorial")
        return

    parser = argparse.ArgumentParser(description="cctutorial - 프로젝트 튜토리얼 생성")
    ...
    if args.command == "help" or (len(sys.argv) > 1 and sys.argv[1] in ("help", "-h")):
        print("cctutorial [run|status|check|help]\n프로젝트 튜토리얼 생성")
        return
