---
name: cctutorial
description: "OAIS 프로젝트의 Codex 환경, oo 스킬, SC 명령어, 플러그인에 대한 튜토리얼 문서를 생성·관리한다 | 출력: `00_doc/tutorial/`"
---

<!-- ccporting:generated-from-upstream -->
<!-- 원본 스킬은 upstream/ 폴더에 보관된다. -->

# cctutorial - 프로젝트 튜토리얼 생성



> OAIS 프로젝트의 Codex 환경, oo 스킬, SC 명령어, 플러그인에 대한 튜토리얼 문서를 생성·관리한다 | 출력: `00_doc/tutorial/`
...
| **하지 않는 것** | API 문서(→ccdoc), 코드 수정(→ccdev) |
...
| `cctutorial help` | 서브명령어 목록 표시 |

| `cctutorial version` | 스킬 버전 정보 (v01) |

| `cctutorial status` | 서브명령어 리스트, 생성된 튜토리얼 현황 |

| `cctutorial check` | references/checklist.md 기반 체크 및 리포팅 |

| `cctutorial show checklist` | 역할 수행 체크리스트 표시 |

| `cctutorial add checklist "항목"` | 체크리스트 항목 추가 | checklist.md |

| `cctutorial run` | 전체 튜토리얼 생성 실행 |

| `cctutorial run --skill <스킬명>` | 특정 oo 스킬 튜토리얼만 생성 |

| `cctutorial run --category <카테고리>` | 특정 카테고리만 생성 (skills/commands/plugins/overview) |

| `cctutorial sync` | 양방향 동기화 — 플래그 기반 스킬↔튜토리얼 현행화 |

| `cctutorial sync --check` | 동기화 필요 항목 확인 (실제 수정 안 함) |

| `cctutorial sync --clear` | 동기화 플래그 초기화 (.omc/sync-flags/ 삭제) |



실행: `uv run python .agents/skills/cctutorial/scripts/ootutorial_run.py`
...
### `cctutorial run --skill <스킬명>` 개별 실행



```

1. .agents/skills/{스킬명}/SKILL.md 읽기
...
`.agents/skills/cctutorial/scripts/ootutorial_run.py` | `.agents/skills/cchelp/SKILL.md` | `00_doc/tutorial/README.md`
