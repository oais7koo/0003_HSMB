...
| 명령어 | 설명 |
|--------|------|
| `ccdesign help` | 서브명령어 목록 표시 |
| `ccdesign version` | 스킬 버전 정보 (v11) |
| `ccdesign status` | 현재 디자인 시스템 상태 (토큰/테마/컴포넌트 현황) |
| `ccdesign token [--theme 테마명\|--all-themes]` | 토큰 파이프라인 실행 (JSON → CSS/SCSS/pen) |
| `ccdesign analyze [이미지경로\|--url URL] [--sample 샘플명]` | 레퍼런스 분석 → 토큰 추출 제안 (이미지 또는 URL) |
| `ccdesign theme [테마명] [--new\|--apply\|--list]` | 테마 생성/적용/목록 |
| `ccdesign component [컴포넌트명] [--platform web\|flutter] [--tool pencil\|code]` | 컴포넌트 생성 |
| `ccdesign generate [화면명] [--theme 테마명]` | 컴포넌트 조합으로 화면 생성 |
| `ccdesign compbuild [컴포넌트\|--all] [--platform web\|flutter]` | 컴포넌트 엘리먼트 넘버링 + 구성요소 라벨링 |
| `ccdesign check [--platform web\|flutter\|senior_world\|all]` | 컴포넌트 정합성 감사 (넘버링·구조·인덱스 불일치 검출) |
| `ccdesign export [pen파일] [--format png\|dart\|both] [--output 경로]` | pen 파일 → PNG 이미지 / Dart 코드 변환 |
| `ccdesign pencil [--theme 테마명] [--file 경로]` | Pencil 새 파일 생성 + 테마 변수 자동 등록 |
| `ccdesign init [--root 경로]` | 새 프로젝트에 디자인 시스템 폴더 구조 + 빈 템플릿 생성 |
...
| 단계 | 명령어 | 방법론 참조 |
|------|--------|------------|
| STEP 1 레퍼런스 분석 | `ccdesign analyze` | d70210_방법론_AI디자인.md |
| STEP 2 토큰 정의/변환 | `ccdesign token` | d70200_방법론_토큰파이프라인.md |
| STEP 3 컴포넌트 생성 | `ccdesign component` | d70210_방법론_AI디자인.md |
| STEP 4 라이브러리 편입 | `ccdesign component --save` | d70220_아토믹디자인.md |
| STEP 5 테마 패키징 | `ccdesign theme --new` | d70230_방법론_테마시스템.md |
| STEP 5.5 테마 컴포넌트 생성 | `ccdesign theme --apply` | 05_components_theme/ |
| STEP 6 화면 생성 | `ccdesign generate` | d70240_방법론_디자인투코드.md |
