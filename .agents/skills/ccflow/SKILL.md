---
name: ccflow
description: "참조: common_guide.md | 튜토리얼: .claude/tutorial/11_SW개발워크플로우.md"
---
...
- 확인 없이 즉시 실행: `ccflow run --yes`

- 단계별 확인: `ccflow run --interactive`
...
| `ccflow version` | 스킬 버전 정보 (v02) | 터미널 |

| `ccflow status` | 현재 SP 워크플로우 진행 현황 | 터미널 |

| `ccflow check` | references/checklist.md 기반 체크 | 터미널 |

| `ccflow run` | dry-run 미리보기 → 확인 → 완전 무인 자동 실행 | 각 스킬 산출물 |

| `ccflow run --yes` | 확인 생략 후 즉시 자동 실행 | 각 스킬 산출물 |
...
| `ccflow run --no-test` | ootest 단계 생략 (빠른 실행 시) | 각 스킬 산출물 |

| `ccflow run --no-review` | ooreview 단계 생략 (빠른 실행 시) | 각 스킬 산출물 |

| `ccflow show checklist` | 역할 수행 체크리스트 표시 | 터미널 |



실행: `uv run python .claude/skills/ccflow/scripts/ooflow_run.py [subcommand] [args]`
...
중단 시 현재까지 완료된 Feature는 커밋 보존됨. `ccflow run`으로 재시작하면 이어서 진행.
