...
| **하지 않는 것** | 코드 수정(→ccdev), E2E 테스트(→cctest), 앱 배포 |
| **참조 범위** | 현재 프로젝트 내부 파일만 / 외부 서버 자동 배포 안 함 |
| **수정 대상** | 캡처 저장 폴더 내 PNG, HTML 파일 |
| **실행 레벨** | [자동] — 빌드→실행→캡처 자동 처리 |
| **에이전트 호환** | Codex 권장 — Playwright MCP 서버 사용 / 다른 에이전트: Playwright 직접 실행으로 대체 |
...
실행: `uv run python .agents/skills/cccapture/scripts/oocapture_run.py [args]`
