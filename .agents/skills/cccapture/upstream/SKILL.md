---
name: cccapture-upstream
description: "Upstream reference for oocapture - 앱 화면 캡처 스킬"
---

# oocapture - 앱 화면 캡처 스킬
...
| 명령어 | 설명 |
|--------|------|
| `oocapture help` | 서브명령어 목록 |
| `oocapture version` | 스킬 버전 (v01) |
| `oocapture run` | 전체 캡처 실행 (빌드 → 서버 → 캡처) |
| `oocapture run --tabs-only` | 메인 탭만 캡처 |
| `oocapture run --port N` | 서버 포트 지정 (기본: 8899) |
| `oocapture run --output DIR` | 출력 경로 지정 |
| `oocapture status` | 현재 캡처 현황 |

실행: `uv run python .claude/skills/oocapture/scripts/oocapture_run.py [args]`
