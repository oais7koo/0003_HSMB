...
def parse_args():
    parser = argparse.ArgumentParser(description="cccapture - 앱 화면 캡처")
...
def flutter_build(flutter_dir):
    ...
    print("[cccapture] Flutter web 빌드 중...")
    ...
    if result.returncode != 0:
        print(f"[cccapture] 빌드 실패:\n{result.stderr}")
        sys.exit(1)
    print("[cccapture] 빌드 완료")
...
def main():
    ...
    if not args.skip_dart_copy:
        print("\n[cccapture] Dart 소스 복사")
        copy_dart_sources(args.flutter_dir, args.output)

    # 3. 캡처
    print(f"\n[cccapture] 캡처 시작 (port={args.port})")
