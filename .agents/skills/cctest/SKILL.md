---
name: cctest
description: "참조: common_guide.md (에이전트) | cccontext.md (SP) | 상세: `references/guide.md`"
---

<!-- ccporting:generated-from-upstream -->
<!-- 원본 스킬은 upstream/ 폴더에 보관된다. -->

> 참조: common_guide.md (에이전트) | cccontext.md (SP) | 상세: `references/guide.md`
...
| 하지 않는 것 | 구현(→ccdev), 정적분석(→cccheck), 이슈수정(→ccfix) |
| 수정 대상 | `tests/TC*.py`, `d{SP}0003_test.md` Part C |
| 실행 레벨 | [자동] PASS까지 반복 (`uv run pytest`) |

## 1. 개요·가이드

TDD RED + 테스트 작성 + 반복 실행. 컨텍스트: `--sp N` 또는 `cccontext N`. 문서연계: 실패→d{SP}0004 / 개발→d{SP}0002 / 항목→d{SP}0003. 역할분리(ccdev=INIT/GREEN/REFACTOR vs cctest=RED/실행/갱신/Part D 재스캔): `references/guide.md` §2.
...
| 명령어 | 설명 |
|--------|------|
| `cctest status` | 서브명령어 리스트, 현재 상태 |
| `cctest check` | references/checklist.md 기반 체크 |
| `cctest run` | 테스트 실행 |
| `cctest show checklist` | 체크리스트 표시 |
| `cctest add checklist "항목"` | 체크리스트 추가 |
...
- **5.5 --runtime (필수)**: py_compile/pylint 미감지 런타임 에러(StreamlitDuplicateElementKey, ImportError, AttributeError, TypeError) 검증. UnboundLocalError 한계는 cccheck `함수 내부 import 스코프 오류 감지`로 대체 — `cccheck run` 선행 필수. Streamlit `pages/*.py` 시 `tests/sp{N}/test_page_import.py` 자동 생성. 상세: guide §18.
- **5.6 checklist**: 5대 차원(완전성/명확성/일관성/측정가능성/커버리지) × 도메인(ux/api/performance/security). 사용: `cctest checklist [domain]`. 상세: guide §19.

## 5. 컴팩트 (--compact)
3KB·테이블/불릿·이력 3개. 상세: guide §20.

## 6. 프레임워크 레퍼런스
FastAPI/Streamlit 감지 시 `.codex/reference/development/{framework}/` 또는 `references/streamlit_guide.md`. 상세: guide §21.

## 7. 관련문서
guide.md | ccdev | cccheck | d{SP}0003 | d{SP}0004 | `.claude/commands/sc/test.md` | `.claude/commands/sc/analyze.md`
