...
| **하지 않는 것** | 코드 에러 이슈 처리(→ccfix), 이슈 발견(→cccheck), 이력 아카이브(→cchistory) |
| **참조 범위** | 현재 프로젝트 내부 파일만 / 외부 프로젝트 자동 포함 안 함 |
| **수정 대상** | `d{SP}0004_todo.md` (대기 ToDo 섹션만), `d{SP}0010_history.md` |
| **실행 레벨** | [반자동] — 코딩 작업은 ccdev 위임, 비코딩 작업은 직접 처리 |
...
- v02 2026-04-17 — `cctodo update` 서브명령어 추가 — todo 문서 현행화 기능
...
**핵심 기능**:
1. **추가+즉시처리**: `cctodo "내용"` -> 추가 후 바로 처리
2. **일괄 처리**: `cctodo` -> 대기 중 업무 전체 처리
3. **추가만**: `cctodo add "내용"` -> 추가만 하고 나중에 처리
4. **상태 조회**: `cctodo status` -> 대기 목록 표시
5. **아카이브**: `cctodo clear` -> 완료 항목을 d0010_history.md로 이동

**옵션**: `--sp N` (서브프로젝트)

### cctodo vs ccfix 역할 구분

- **cctodo**: d0004의 "대기 ToDo" 섹션을 관리. 사용자가 직접 추가한 할 일을 처리하며, 코딩(ccdev 위임)과 비코딩(문서, 분석 등 직접 처리) 모두 대응하는 **범용 할 일 관리자**.
...
| 명령어 | 설명 |
|--------|------|
| `cctodo help` | 서브명령어 목록 표시 |
| `cctodo version` | 스킬 버전 정보 (v09) |
| `cctodo "내용"` | **추가 + 즉시 처리** (권장) |
| `cctodo` | 대기 중 업무 전체 처리 |
| `cctodo list` | **전체 서브프로젝트 대기 업무 리스트업** |
| `cctodo status` | 서브명령어 리스트, 대기 목록 표시 |
| `cctodo check --checklist` | references/checklist.md 기반 체크 및 리포팅 | 터미널 |
| `cctodo run` | TODO 처리 실행 |
| **`cctodo run this`** | **직전 작업 TODO 처리** (→ common_guide.md §9) |
| `cctodo show checklist` | 역할 수행 체크리스트 표시 | 터미널 |
| `cctodo add checklist "항목"` | 체크리스트 항목 추가 | checklist.md |
| `cctodo add [text]` | 할 일 추가만 (처리 안함) |
| `cctodo complete [ID]` | 대기 항목 → 완료 ToDo 섹션으로 이동 (완료일 자동 기록) |
| `cctodo clear` | 완료 항목 -> d0010_history.md 아카이브 |
| **`cctodo check`** | **등록 이슈 타당성 검토 — 전문 에이전트가 유효성·우선순위·중복 분석 후 의견 제시** (구 `validate`) |
| `cctodo check --sp N` | 특정 SP의 이슈만 검토 |
| `cctodo check --fix` | 검토 결과 기반으로 d{SP}0004 자동 보정 (우선순위 조정·중복 병합) |
| **`cctodo update`** | **d{SP}0004_todo.md 현행화** — 완료 항목 상태 갱신·문서이력 버전 업데이트·우선순위 재정렬 |
| `cctodo update --message "msg"` | 이력 변경 내용 직접 지정 |
| `cctodo update --no-sort` | 우선순위 재정렬 생략 |
| `cctodo update --dry-run` | 변경사항 미리보기 (파일 수정 안함) |
| **`cctodo audit`** | **완료 항목 검증** — 완료 ToDo 항목별 검증 지침 출력 후 `history`/`reopen`으로 판정 |
| `cctodo audit --sp N` | 특정 SP의 완료 항목만 검증 |
| `cctodo history [ID]` | 완료 항목을 d0010에 단건 아카이브 + d0004에서 제거 |
| `cctodo reopen [ID] "메모"` | 완료 항목을 대기 ToDo로 복원 + 재개 메모 추가 |

실행: `uv run python .agents/skills/cctodo/scripts/ootodo_run.py [args]`
...
- 블록 간 **빈 줄 1개** 유지
- `cctodo add` → 대기 블록 생성, `cctodo complete [ID]` → 완료일 자동 기록 후 완료 섹션으로 이동
...
|:-------:|------------|----------|----------|
| 1 | PRD, 요구사항, 기획서 | `ccprd` | `ccprd run` |
| 2 | plan, 계획, WBS, 스프린트, Feature | `ccplan` | `ccplan run` |
| 3 | 상세기획, 상세설계, 기능 문서, d{N}1001 | `ccfeature` | `ccfeature next dXXXX` |
| 4 | 구현, 개발, 코딩, TDD, 함수, 클래스, API | `ccdev` | `ccdev run dXXXX` |
| 5 | 버그, 오류, 수정, fix, 에러 | `ccfix` | `ccfix run` |
| 6 | 테스트, TC, 검증, pytest | `cctest` | `cctest run` |
| 7 | 코드 체크, 품질, pylint, lint | `cccheck` | `cccheck run` |
| 8 | 문서, ccdoc, d0001~d0010 | `ccdoc` | `ccdoc run` |
| 9 | 환경, 패키지, uv, 라이브러리 | `ccuv` / `ccenv` | `ccuv add` / `ccenv run` |
| 10 | 커밋, git, 이력 | `cccommit` | `cccommit run` |
| 11 | 논문, 스크래핑, 크롤링, 유튜브 | `ccpaper` / `ccscrap` | 해당 스킬 `run` |
| 12 | (키워드 미매칭) | 직접 처리 | AI가 내용 보고 판단 |

**스킬 실행 전 확인 사항**:
- `dXXXX` 가 필요한 스킬(ccfeature, ccdev, cccheck)은 todo 내용에서 문서번호 추출 시도
- 추출 실패 시 사용자에게 문서번호 질문 또는 `ccfeature needed`로 파악
...
### 4.5.1 check 워크플로우 (`cctodo check`)
...
**cccommit sync와의 차이**:
- `cctodo update`: d{SP}0004 내부 현행화 (상태·이력·정렬) — **파일 저장만**
- `cccommit sync`: d{SP}0004 완료 항목 → d{SP}0010 이동 — **이력 이동 포함**
- 권장 순서: `cctodo update` → `cccommit run` (커밋 + sync 통합)

---

### 4.7 완료 항목 아카이브 (`cctodo clear`)
...
> **관련 문서**: `.agents/skills/cctodo/references/guide.md` | `.agents/skills/cctodo/templates/ootodo_template.md` | `.agents/skills/ccdev/SKILL.md` | `.agents/skills/cchistory/SKILL.md` | `00_doc/d{SP}0004_todo.md` 외
