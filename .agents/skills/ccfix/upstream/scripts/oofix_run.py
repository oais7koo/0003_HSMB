...
명령어:
    oofix run [대상]     - 이슈 자동 수정
    oofix preview        - 수정 미리보기 (dry-run)
    oofix test [대상]    - 테스트 시나리오 실행
    oofix verify [파일]  - 수정 검증
    oofix rollback       - 마지막 수정 롤백
...
def print_usage():
    ...
    print("oofix - 코드 오류 자동 수정 도구")
    print()
    print("사용법:")
    print("    oofix run              전체 이슈 자동 수정")
    print("    oofix run [이슈ID]     특정 이슈만 수정 (예: TODO-001)")
    print("    oofix run [카테고리]   카테고리별 수정 (CRITICAL/ERROR/WARNING)")
    print("    oofix preview          수정 미리보기 (실제 수정 없음)")
    print("    oofix preview [이슈ID] 특정 이슈 미리보기")
    print("    oofix test             전체 테스트 시나리오 실행")
    print("    oofix test [시나리오]  특정 시나리오만 실행 (예: TC-001)")
    print("    oofix verify           최근 수정 파일 검증")
    print("    oofix verify [파일]    특정 파일 검증")
    print("    oofix rollback         마지막 수정 롤백")
    print("    oofix status           이슈 현황 조회")
    print("    oofix status [카테고리] 카테고리별 상태")
    print()
    print("예시:")
    print("    python .claude/skills/oofix/scripts/oofix_run.py run")
    print("    python .claude/skills/oofix/scripts/oofix_run.py preview")
    print("    python .claude/skills/oofix/scripts/oofix_run.py test TC-001")
    print("    python .claude/skills/oofix/scripts/oofix_run.py status CRITICAL")
...
def cmd_dart_status(target_dir):
    ...
    print("# oofix status (Flutter/Dart)\n")
