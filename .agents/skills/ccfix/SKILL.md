...
| 명령어 | 설명 |
|--------|------|
| `ccfix help` | 서브명령어 목록 표시 |
| `ccfix version` | 스킬 버전 정보 (v01) |
| `ccfix status` | 서브명령어 리스트, 이슈 상태 |
| `ccfix check` | references/checklist.md 기반 체크 및 리포팅 |
| ccfix show checklist | 역할 수행 체크리스트 표시 |
| ccfix add checklist "항목" | 체크리스트 항목 추가 |
| ccfix run | 이슈 자동 수정 (병렬) |
| **ccfix run this** | **직전 작업 파일 이슈 수정** (→ common_guide.md §9) |
| ccfix run [대상] | 특정 이슈/파일/카테고리 |
| ccfix improve [대상] | d0004 이슈 없이 품질/성능/보안 개선 (improve 흡수) |
| ccfix preview | 수정 미리보기 |
| ccfix test | 테스트 실행 |
| ccfix verify | 수정 검증 |
| ccfix rollback | 롤백 |
