...
이 스킬은 **`ccstart`의 약어 alias**입니다.

전달된 인자(`$ARGUMENTS`)를 그대로 유지하여 **즉시 `ccstart` 스킬을 실행**하세요.
Skill 도구로 `ccstart`를 호출하고 `$ARGUMENTS`를 args로 전달합니다.
...
| 항목 | 내용 |
|------|------|
| **핵심 역할** | `ccstart` 스킬의 약어 alias — 세션 시작 워크플로우 |
| **하는 것** | `ccstart` 스킬 실행 + 완료 후 SP 선택 프롬프트 |
| **하지 않는 것** | 파일 수정, 코드 실행 |
| **에이전트 호환** | 범용 — `ccstart`을 직접 호출하는 것으로 대체 가능 |

## 실행 후 처리 (필수)

`ccstart` 스크립트 실행이 완료되면 **반드시** 아래 순서로 처리하세요:
...
4. 사용자 입력값 N을 받아 즉시 `cccontext N` 실행 (Skill 도구로 `cccontext` 호출, args=N)
5. Enter(빈 입력) 또는 Active SP 없음 → 현재 컨텍스트 유지 (cccontext 호출 생략)
