...
def cmd_status():
    ...
    print("| 명령어 | 설명 |")
    print("|--------|------|")
    print("| `ccskill status`        | 현황 요약 (이 화면) |")
    print("| `ccskill version`       | 버전 정보 (v01) |")
    print("| `ccskill validate`      | 서브에이전트/명령어 최적화 검증 |")
    print("| `ccskill run`           | 전체 스킬 자동 최적화 |")
    print("| `ccskill run [스킬명]`  | 특정 스킬만 최적화 |")
    print()
    ...
    if skill_files:
        ...
        print()
        print(f"[TIP] `ccskill validate` 로 상세 분석 / `ccskill run` 으로 자동 최적화")
...
def cmd_validate(skill_name: str = None):
    ...
    print(f"[TIP] `ccskill run` 으로 자동 최적화 적용 가능")
    return 0
...
def cmd_validate_checklist(fix: bool = False):
    ...
    import re as _re
    print("[ccskill validate-checklist]\n")
...
def cmd_update(dry_run: bool = False):
    ...
    print()
    if dry_run:
        print("[DRY-RUN] 실제 파일 수정 없음. `ccskill update` 로 적용하세요.")
    else:
...
def cmd_show_checklist():
    ...
    checklist = SKILLS_DIR / "ccskill" / "references" / "checklist.md"
    if not checklist.exists():
...
def cmd_check():
    """check - checklist.md 기반 ccskill 자체 건강 상태 점검"""
    checklist_path = SKILLS_DIR / "ccskill" / "references" / "checklist.md"
    print("[ccskill check]\n")

    results = []

    # C01 필수 파일 존재
    skill_md = SKILLS_DIR / "ccskill" / "SKILL.md"
    ...
    if skill_md.exists():
        ...
        ver_table = re.search(r'`ccskill version`.*\(v(\d+)\)', content)
    ...
    # C12 help 스크립트화
    run_py = SKILLS_DIR / "ccskill" / "scripts" / "ooskill_run.py"
    if run_py.exists():
...
def cmd_run(skill_name: str = None, dry_run: bool = False):
    ...
    if results:
        ...
        print("[TIP] `ccskill validate` 로 수동 개선 권장사항 확인")

    # 분리 원칙: run은 배치 실행만 수행. 현행화는 `ccskill update`로 별도 실행.
    if modified > 0:
        print()
        print("[TIP] CLAUDE.md 카탈로그 및 기타 현행화는 `ccskill update` 실행")
...
def main():
    ...
    if cmd == "status":
        ...
        if target is None:
            print(f"# ccskill update (전체 현행화){'  [DRY-RUN]' if dry_run else ''}")
            print()
        ...
        _print_skill_help("ccskill")
        return 0
    else:
        log_err(f"알 수 없는 서브명령어: {cmd}")
        show_help_if_no_args("ccskill", [])
        return 1
