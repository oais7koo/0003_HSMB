...
| 명령어 | 설명 | 출력 |
|--------|------|------|
| `ccprd help` | 서브명령어 목록 | 터미널 |
| `ccprd version` | 스킬 버전 (v09) | 터미널 |
| `ccprd status` | 서브명령어 리스트, 상태 요약 | 터미널 |
| `ccprd check` | references/checklist.md 기반 체크 | 터미널 |
| `ccprd run` | PRD 생성 실행 | 터미널 |
| `ccprd run --with-update` | PRD 생성 후 현행화 | 터미널 |
| `ccprd show checklist` | 역할 체크리스트 표시 | 터미널 |
| `ccprd add checklist "항목"` | 체크리스트 항목 추가 | checklist.md |
| ccprd run | PRD 생성/정합성 검증 | d{SP}0001_prd.md |
| `ccprd update` | 현행화 — 코드/현황 스캔 → PRD 반영 | d{SP}0001_prd.md |
| `ccprd update --dry-run` | 변경 예정 미리 출력 | 터미널 |
| **`ccprd run this`** | **직전 작업 PRD 섹션 갱신** (common_guide.md §9) | d{SP}0001_prd.md |
| ccprd run --template [type] | 템플릿 지정 생성 | d{SP}0001_prd.md |
| ccprd check --fix | PRD 최적화 (구 `optimize`) | d{SP}0001_prd.md |
| ccprd template | 템플릿 목록 | 터미널 |
| ccprd template [type] | 특정 템플릿 조회 | 터미널 |
| ccprd check --structure | 구조 검증 (구 `validate`) | 터미널 |
| ccprd section [N] | 특정 섹션 갱신 | d{SP}0001_prd.md |
| ccprd unitdev | 전체 단위개발문서 현행화 | d{SP}1XXX_*.md |
| ccprd unitdev [문서명] | 특정 단위개발문서 현행화 | 해당 문서 |
| ccprd clarify | PRD 모호성 해소 (최대 5개 질문) | d{SP}0001_prd.md |
...
| 시나리오 | oo 스킬 | GSD 명령어 |
|---------|---------|-----------|
| 신규 프로젝트 초기화 | `ccprd run` (신규) | `/gsd:new-project` |
| 신규 마일스톤 추가 | `ccprd run --template` | `/gsd:new-milestone` |
| 요구사항 모호성 해소 | `ccprd clarify` | `/gsd:discuss-phase [N]` |
