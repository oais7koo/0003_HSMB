...
| 명령어 | 설명 |
|--------|------|
| `ccnext help` | 서브명령어 목록 표시 |
| `ccnext version` | 스킬 버전 정보 (v01) |
| `ccnext status` | 서브명령어 리스트, 스킬 상태 |
| `ccnext check` | references/checklist.md 기반 체크 및 리포팅 |
| `ccnext show checklist` | 역할 수행 체크리스트 표시 |
| `ccnext add checklist "항목"` | 체크리스트 항목 추가 |
| `ccnext run` | **현재 컨텍스트 기준 다음 작업 추천 실행 (기본)** |
| **`ccnext run this`** | **last_target 기반 다음 작업 추천** (→ common_guide.md §9) |
| `ccnext run --sp N` | 특정 서브프로젝트 대상으로 추천 |
