#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""ccdoc check - 문서 정합성 통합 검사

Part 1: d0001~d0010 교차 정합성 (문서 간 일관성 검증)
Part 2: SP 폴더 귀속 검사 (파일이 올바른 SP 폴더에 있는지)

사용법:
    uv run python .claude/skills/ccdoc/scripts/oodoc_check.py
    uv run python .claude/skills/ccdoc/scripts/oodoc_check.py sp04
    uv run python .claude/skills/ccdoc/scripts/oodoc_check.py --fix
"""
...
# 프로젝트 루트: scripts/ → ccdoc/ → skills/ → .claude/ → D:/1_oo/
...
def main():
    parser = argparse.ArgumentParser(description="ccdoc check - 문서 정합성 통합 검사")
