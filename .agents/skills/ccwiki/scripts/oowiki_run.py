...
def cmd_status():
    if not WIKI_DIR.exists():
        print(f"[ccwiki status] 위키 폴더 없음: {WIKI_DIR}")
...
def cmd_list():
    ...
    print(f"[ccwiki list] 총 {len(md_files)}개 페이지\n")
    current_cat = None
    for f in md_files:
...
def cmd_lint():
    ...
    print(f"[ccwiki lint] 점검 완료 — {len(md_files)}개 페이지\n")
    if issues:
...
def cmd_index():
    ...
    print(f"[ccwiki index] index.md 재생성 완료 — {len(md_files)}개 페이지 등록")


def cmd_inbox_list() -> list:
    """0019_정리 폴더의 처리 대기 파일 목록 출력"""
    if not INBOX_DIR.exists():
        print(f"[ccwiki inbox] 정리 폴더 없음: {INBOX_DIR}")
        return []

    all_files = sorted(f for f in INBOX_DIR.rglob("*") if f.is_file())
    if not all_files:
        print("[ccwiki inbox] 0019_정리 폴더가 비어 있습니다.")
        return []

    print(f"[ccwiki inbox] 처리 대기 파일: {len(all_files)}개\n")
    for f in all_files:
        size = f.stat().st_size
...
def cmd_inbox_purge_binary() -> int:
    ...
    if deleted:
        print(f"[ccwiki inbox] 바이너리 파일 {len(deleted)}개 자동 삭제:")
        for name in deleted:
            print(f"  - {name}")
    return len(deleted)
...
def cmd_inbox_clear(targets: list | None = None):
    ...
    print(f"[ccwiki inbox] {len(deleted)}개 파일 삭제 완료 (0019_정리 폴더 유지)")
    if failed:
...
def main():
    ...
    if not args or args[0] == "help":
        ...
        if inbox_files:
            print("\n[ccwiki run] 위 inbox 파일들을 0020_wiki에 통합 후 'ccwiki inbox --clear'로 삭제하세요.")
        # qmd 임베딩
        print("\n[ccwiki run] qmd embed 실행 중...")
        import subprocess, shutil
        ...
        if result.returncode != 0:
            print("[ccwiki run] qmd embed 실패. 직접 실행: powershell -Command 'qmd embed'")
    elif args[0] == "extract":
        cmd_inbox_extract()
    elif args[0] == "convert":
        cmd_inbox_convert()
