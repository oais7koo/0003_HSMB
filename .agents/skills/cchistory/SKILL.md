...
| **하지 않는 것** | Git 커밋(→cccommit), 이슈 수정(→ccfix), 커스텀 Todo 처리(→cctodo) |
...
> **스코프 경계**: oohistory는 **문서 이력 아카이브** 전담 (d0004→d0010). git 커밋/푸시는 `cccommit` 담당.

| 기능 | 설명 |
|------|------|
| `cchistory help` | 서브명령어 목록 표시 |
| `cchistory version` | 스킬 버전 정보 (v01) | 터미널 |
...
| 명령어 | 설명 | 출력 |
|--------|------|------|
| `cchistory status` | 서브명령어 리스트, 이동 대상 항목 수, d0010 토큰 상태 | - |
| `cchistory check` | references/checklist.md 기반 체크 및 리포팅 | 터미널 |
| `cchistory show checklist` | 역할 수행 체크리스트 표시 | 터미널 |
| `cchistory add checklist "항목"` | 체크리스트 항목 추가 | checklist.md |
| `cchistory run` | 완료 항목 d{SP}0010 이동 | 00_doc/d{SP}0010_history.md |
| **`cchistory run this`** | **직전 완료 이슈 아카이브** (→ common_guide.md §9) | 00_doc/d{SP}0010_history.md |

실행: `uv run python .agents/skills/cchistory/scripts/oohistory_run.py [args]`
...
`cchistory run --compact` 또는 `ccdoc run --compact` 호출 시 적용. guide.md 템플릿보다 우선:
...
- **태그**: .agents/skills/cctodo/references/guide.md
