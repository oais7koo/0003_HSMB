...
| 명령어 | 설명 |
|--------|------|
| `cchwp help` | 서브명령어 목록 표시 |
| `cchwp version` | 스킬 버전 정보 (v01) |
| `cchwp status` | 서브명령어 리스트, 현재 상태 |
| `cchwp check --checklist` | references/checklist.md 기반 체크 및 리포팅 | 터미널 |
| `cchwp build` | 템플릿 기반 HWPX 생성 |
| `cchwp extract` | HWPX에서 텍스트 추출 |
| `cchwp check` | HWPX 구조 검증 (구 `validate`) |
| `cchwp analyze` | 기존 HWPX 구조 분석 |
| `cchwp edit` | 기존 HWPX 편집 (unpack → 수정 → pack) |
| `cchwp convert <input.hwp> [output.hwpx]` | HWP → HWPX 변환 (pyhwpx, Windows + 한글 필요) |
| `cchwp convert --batch <dir>` | 폴더 내 모든 .hwp 일괄 변환 |
| `cchwp pagecount <dir>` | 폴더 내 문서 페이지 카운트 (HWP/HWPX/PDF 자동 감지) |
