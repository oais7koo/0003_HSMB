...
| 명령어 | 설명 |
|--------|------|
| `ccsota help` | 서브명령어 목록 표시 |
| `ccsota version` | 스킬 버전 정보 (v01) |
| `ccsota show checklist` | 역할 수행 체크리스트 표시 |
| `ccsota add checklist "항목"` | 체크리스트 항목 추가 |
| `ccsota status` | 현재 논문 초안 현황, 실험 문서 목록 |
| `ccsota init` | 논문 구조 분석 → 섹션 골격 생성 (d{SP}6200_paper_draft.md) |
| `ccsota run` | 전체 논문 초안 생성/업데이트 |
| `ccsota section [섹션]` | 특정 섹션만 작성/수정 |
| `ccsota review` | **논문 종합 교정** (논리 흐름, SOTA 비교, 일관성 + P01~P21 교정 체크리스트 통합). 기준: `paper_quality_checklist.md` 전체 적용 |
| `ccsota table` | 실험 결과 테이블 자동 생성/업데이트 |
| `ccsota figures` | Figure 목록 및 캡션 계획 생성 |
| `ccsota submit [저널]` | 투고 저널 형식 검토 및 체크리스트 |
| `ccsota diff [v1] [v2]` | 논문 버전 간 변경 내용 비교 |
| `ccsota check` | 논문 품질 체크리스트 점검 (섹션 구조, 표 도입문 누락, 수치 일관성, 내부 마커 잔존 등) |
**섹션 지정** (`ccsota section` 대상):
