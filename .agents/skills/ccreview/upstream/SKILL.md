---
name: ooreview
description: "코드 리뷰 스킬. 'ooreview', '코드 리뷰', '리뷰 요청', 'code review', '설계 검토', '보안 검토' 키워드로 트리거. oocheck(정적 분석)와 달리 설계·패턴·보안·성능을 판단 기반으로 검토. code-reviewer(opus) + security-reviewer + quality-reviewer 병렬 실행 후 Codex CLI로 2차 의견 수렴. oocheck → ooreview → oofix 흐름에서 사용."
metadata:
  version: v02
  category: core-dev
---
...
> 상세 체크리스트: .claude/skills/ooreview/references/checklist.md
...
> ※ **codex:rescue 사용 주의**: codex 플러그인은 자동화 가능한 에이전트로 `codex-rescue` 하나만 제공한다. 이름은 "rescue"(수정)지만 프롬프트에 "review" 또는 "read-only"를 명시하면 read-only 리뷰 모드로 동작한다(에이전트 정의 L34 기준). ooreview는 **수정 없이 2차 의견만** 받기 위해 read-only 프롬프트로 호출한다. `/codex:review` 슬래시 명령은 인터랙티브 전용이므로 자동화 파이프라인에서는 사용하지 않는다.

> ⚠️ **필수**: 리뷰는 반드시 Agent 도구로 위임할 것. 직접 처리 금지.
> 순서: explore(haiku) 스캔 → 3-way 병렬 리뷰 → Codex 2차 의견(read-only) → 결과 합산
...
| 명령어 | 설명 |
|--------|------|
| `ooreview help` | 서브명령어 목록 |
| `ooreview run` | 현재 SP 전체 리뷰 |
| `ooreview run [파일/디렉토리]` | 특정 대상만 리뷰 |
| `ooreview run this` | 직전 작업 파일만 리뷰 (common_guide.md §9) |
| `ooreview run dXXXX` | 상세 문서 기반 관련 코드만 리뷰 |
| `ooreview security` | 보안 집중 리뷰 (security-reviewer + Codex) |
| `ooreview design` | 아키텍처/설계 집중 리뷰 (architect + code-reviewer) |
| `ooreview codex [파일]` | Codex 단독 2차 리뷰 |
| `ooreview status` | 미해결 리뷰 이슈 현황 |
...
| 시나리오 | 명령어 |
|---------|--------|
| 페이즈 완료 전 리뷰 | `ooreview run` → `/gsd:validate-phase` |
| PR 전 종합 리뷰 | `ooreview run` → `oocommit run` |
| 보안 감사 | `ooreview security` → `/gsd:forensics` |
...
| Codex 2차 리뷰 | codex:rescue (read-only 모드) / omc-teams codex | - | 엣지케이스·로직 결함 2차 의견 (수정 없음, 프롬프트에 "review" 명시 시 rescue가 read-only로 동작) | O |
