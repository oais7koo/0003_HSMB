---
name: ccsidi
description: "01_obsidian/ 볼트의 마크다운 문서 생성·편집·검색·구조 관리 스킬 | ref: `.codex/guides/common_guide.md`"
---

<!-- ccporting:generated-from-upstream -->
<!-- 원본 스킬은 upstream/ 폴더에 보관된다. -->

# ccsidi - 옵시디언 볼트 문서 관리

> 01_obsidian/ 볼트의 마크다운 문서 생성·편집·검색·구조 관리 스킬 | ref: `.codex/guides/common_guide.md`
...
| **하지 않는 것** | 연구노트(→ccnote), 프로젝트 문서(→ccdoc), 소개서 작성(→ccsidi [소개서]) |
...
| 명령어 | 설명 |
|--------|------|
| `ccsidi help` | 서브명령어 목록 표시 |
| `ccsidi version` | 스킬 버전 정보 (v01) |
| `ccsidi status` | 볼트 폴더별 문서 수, 전체 현황 |
| `ccsidi run` | 상태 점검 + 리포트 (기본) |
| `ccsidi list <폴더>` | 특정 폴더의 문서 목록 |
| `ccsidi create <폴더> "<제목>"` | 새 문서 생성 |
| `ccsidi index <폴더>` | 폴더 인덱스 파일 갱신 |
| `ccsidi search "<키워드>"` | 볼트 전체 키워드 검색 |
| `ccsidi init-topic <폴더> "<주제>"` | 주제 서브폴더 + 개요 5종 세트 생성 |
| `ccsidi init-folder <NNNN> "<이름>"` | 최상위 폴더 + 인덱스 + 개요 5종 세트 |
| `ccsidi links <파일>` | 문서의 위키링크 연결 상태 확인 |
| `ccsidi check` | references/checklist.md 기반 전체 체크 |
| `ccsidi check <NNNN>` | 해당 번호로 시작하는 폴더만 C05/C06 체크 (예: `check 0004`) |

실행: `uv run python .agents/skills/ccsidi/scripts/oosidi_run.py [args]`
...
> `ccsidi init-folder`는 `01_개요/` + 5종 세트를 자동 생성. `02_개념/`은 수동 생성.
...
`ccsidi 정리` 요청 시 아래 포맷을 기준으로 정리한다.
...
`.agents/skills/ccsidi/scripts/oosidi_run.py` | `.agents/skills/ccdoc/SKILL.md` | `01_obsidian/`
