#!/usr/bin/env python3
"""ccsidi - 옵시디언 볼트 문서 관리 스킬 메인 스크립트"""
...
def cmd_status(vault: Path):
    """볼트 상태 리포트"""
    print("# ccsidi Vault Status Report\n")
...
def cmd_check(vault: Path, folder_prefix: str = None):
    ...
    """
    if folder_prefix:
        print(f"[ccsidi check {folder_prefix}]\n")
    else:
        print("[ccsidi check]\n")
...
def cmd_help():
    """도움말"""
    print("# ccsidi - 옵시디언 볼트 문서 관리\n")
    print("Usage: ccsidi <command> [args]\n")
    print("Commands:")
    print("  help                              서브명령어 목록")
    print("  version                           버전 정보")
