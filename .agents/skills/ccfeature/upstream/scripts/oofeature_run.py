#!/usr/bin/env python3
"""
oofeature_run.py - oofeature 스킬 스크립트 (상세 문서 생명주기 관리)
Usage: uv run python .claude/skills/oofeature/scripts/oofeature_run.py [help|version|status]
"""
import sys
...
def cmd_update(args):
    import subprocess
    script = _SKILLS_DIR / "oofeature" / "scripts" / "oofeature_update.py"
...
def cmd_note(args):
    import subprocess
    script = _SKILLS_DIR / "oofeature" / "scripts" / "oofeature_note.py"
    if not script.exists():
...
def cmd_issue(args):
    import subprocess
    script = _SKILLS_DIR / "oofeature" / "scripts" / "oofeature_issue.py"
    if not script.exists():
...
def cmd_rmdup(args):
    import subprocess
    script = _SKILLS_DIR / "oofeature" / "scripts" / "oofeature_rmdup.py"
    if not script.exists():
...
def cmd_stage(args):
    # usage: oofeature stage dXXXX 단계
    if len(args) < 3:
        print("[ERROR] 사용법: oofeature stage dXXXX 단계")
