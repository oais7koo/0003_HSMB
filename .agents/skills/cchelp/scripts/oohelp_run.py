#!/usr/bin/env python
"""cchelp - OAIS 스킬 도움말 자동 생성 스크립트

.agents/skills/oo*/SKILL.md 파일을 스캔하여 스킬 목록과 서브명령어를 자동 추출합니다.

Usage:
    uv run .agents/skills/cchelp/scripts/oohelp_run.py           # 전체 스킬 목록
    uv run .agents/skills/cchelp/scripts/oohelp_run.py ccdev   # 특정 스킬 상세
    uv run .agents/skills/cchelp/scripts/oohelp_run.py --json    # JSON 출력
"""
...
SKILL_CATEGORIES = {
    "컨텍스트": ["cccontext"],
    "환경": ["ooenv", "ccstart", "ccstop"],
    "기획": ["ccprd", "ccplan"],
    "개발": ["ccdev"],
    "품질": ["cccheck", "ccfix", "cctest"],
    "모듈/DB": ["cclib", "ccdb"],
    "문서": ["ccdoc", "cchistory", "cccommit"],
    "보고서": ["ccppt", "ccreport"],
    "도움말": ["cchelp"],
}

def get_skill_files() -> list[Path]:
    """.agents/skills/oo*/SKILL.md 파일 목록 반환"""
...
def print_skill_list(skills: list[dict]) -> None:
    ...
    print("**일일 개발**")
    print("```")
    print("cccontext 02 → ccdev status → ccdev run → cccheck → ccfix run → cccommit")
    print("```")
    print()
    print("**전체 파이프라인**")
    print("```")
    print("ccflow run: prd → plan → feature → dev → check → fix → commit")
    print("```")
    print()
    print("---")
    print()
    print("**상세 도움말**: `uv run .agents/skills/cchelp/scripts/oohelp_run.py [스킬명]`")
...
def print_skill_detail(skill: dict) -> None:
    ...
    if skill["all_commands"]:
        ...
        print("- (서브명령어 없음)")
    print()
    print(f"**상세 문서**: `.agents/skills/{skill['name']}/SKILL.md`")
...
def main():
    ...
    if show_help_if_no_args("cchelp", args):
        return
