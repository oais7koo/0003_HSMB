...
| **하지 않는 것** | 코드 실행(→ccrun), 환경 설치(→ooenv), 문서 동기화(→ccsync) |
...
| **에이전트 호환** | Codex 권장 — `uv run` 기반 인터랙티브 실행 |
...
| `ccdata version` | 스킬 버전 정보 (v04) |
| `ccdata status` | 스킬 상태 및 경로 정보 표시 |
| `ccdata run` | data/ 서브폴더 스캔 → d0007_data.md 생성/업데이트 |
| `ccdata comment <폴더명> "<메모>"` | 특정 서브폴더 설명에 메모 추가 → d0007_data.md 업데이트 |
...
실행: `uv run python .agents/skills/ccdata/scripts/oodata_run.py [서브명령어]`
...
1. `d0007_data.md` 로드 (없으면 `ccdata run` 먼저 실행 안내)
2. 지정 서브폴더 행 탐색
3. 해당 폴더 설명에 메모 내용 추가 (기존 설명 보존, 뒤에 덧붙임)
4. `d0007_data.md` 저장

```bash
ccdata comment ps1000_원본데이터 "2026-05-14 재생성 불가 확인됨"
ccdata comment ps3000_실험결과 "최종 제출본 포함"
```
...
| 문서 | 용도 |
|------|------|
| `00_doc/sp00/d0007_data.md` | **data/ 폴더 구조·설명 문서** (SP 무관 단일 문서, ccdata 관리 전용, 번호 0007 예약) |
