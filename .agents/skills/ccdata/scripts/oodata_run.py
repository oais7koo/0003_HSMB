...
Usage: uv run python .agents/skills/ccdata/scripts/oodata_run.py [run|comment|backup|restore|list|status|help]
"""
import sys
import shutil
...
# 프로젝트 루트: scripts/ → ccdata/ → skills/ → .codex/ → project_root
...
def build_data_doc(entries: list, descriptions: dict) -> str:
    ...
    lines = [
        "# data/ 폴더 구조 (프로젝트 공통)",
        "",
        "> data/ 는 SP에 무관한 프로젝트 루트 공통 폴더이며, 본 문서는 ccdata 스킬이 관리한다.",
        "",
        "## 문서 이력 관리",
        f"- {now} — ccdata run 자동 생성/업데이트",
        "",
        "## 서브폴더 목록",
        "",
        "| 폴더 | 상태 | 설명 |",
...
def main():
    ...
    if cmd == "comment":
        if len(args) < 3:
            print('[ERROR] 사용법: ccdata comment <폴더명> "<메모>"')
            return
