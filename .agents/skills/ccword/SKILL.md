---
name: ccword
description: "공통: `.codex/guides/common_guide.md` | 가이드: `.agents/skills/ccword/references/guide.md`"
---

<!-- ccporting:generated-from-upstream -->
<!-- 원본 스킬은 upstream/ 폴더에 보관된다. -->

> 공통: `.codex/guides/common_guide.md` | 가이드: `.agents/skills/ccword/references/guide.md`
...
| **하지 않는 것** | PPT 생성(→ccppt), PDF 변환(→ccpdf), HWP 생성(→cchwp) |
| **참조 범위** | 현재 프로젝트 내부 파일만 / 외부 문서 서비스 자동 연동 안 함 |
| **수정 대상** | 출력 `*.docx` 파일 |
| **실행 레벨** | [자동] — 변환 자동 실행 |
| **에이전트 호환** | Codex 권장 — python-docx/pandoc `uv run` 자동 실행 / 다른 에이전트: `pandoc input.md -o output.docx` 수동 실행 |
...
| 명령어 | 설명 |
|--------|------|
| `ccword help` | 서브명령어 목록 표시 |
| `ccword version` | 스킬 버전 정보 (v02) |
| `ccword status` | 서브명령어 리스트, 현재 상태 |
| `ccword check` | references/checklist.md 기반 체크 및 리포팅 | 터미널 |
| `ccword run` | Word 문서 생성 실행 |
| `ccword update` | 현행화 — 소스 변경분 감지 → Word 문서 재생성 | .docx |
| `ccword update --dry-run` | 변경 예정 내용 미리 출력 (실제 수정 안 함) | 터미널 |
| `ccword show checklist` | 역할 수행 체크리스트 표시 | 터미널 |
| `ccword add checklist "항목"` | 체크리스트 항목 추가 | checklist.md |
...
| 스킬 | 용도 | 필수 |
|------|------|:----:|
| ccreport | MD->DOCX 변환 스크립트 (`ccreport_md2docx.py`) 제공 | O |

## 관련 파일

| 구분 | 경로 |
|------|------|
| 스킬 정의 | `.agents/skills/ccword/SKILL.md` |
| 변환 스크립트 | `.agents/skills/ccreport/scripts/ooreport_md2docx.py` |
| 견적서 변환기 | `.agents/skills/ccword/templates/quotation_docx.js` |
| 견적서 스타일 | `.agents/skills/ccword/templates/quotation_docx.json` |
| 가이드 | `.agents/skills/ccword/references/guide.md` |
