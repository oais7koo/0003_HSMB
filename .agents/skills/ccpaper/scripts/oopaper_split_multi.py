...
주의:
    - suffix 부착(-NN) 절대 금지
    - 분리 후 paper_list 동기화는 `ccpaper update --lang en` 수동 실행
...
def cmd_split(args) -> int:
    ...
    print(f"\n다음 단계 권장:")
    print(f"  1. `ccpaper update --lang {args.lang}` — paper_list 동기화")
    print(f"  2. `ccpaper run --folder <NEW_ID>` — 신규 폴더별 산출물 생성")
    return 0
