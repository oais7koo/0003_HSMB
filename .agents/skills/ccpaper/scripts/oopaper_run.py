...
def determine_status(result):
    """5단계 상태 판정 (v05): X/E/S/T/O — 영문→서머리→번역 순서"""
...
def _phase0_organize(dry_run: bool) -> int:
    """Phase 0: 00_down PDF → YYMMDD-HHMM 폴더 이동 + 파일명 정규화."""
    if not DOWN_DIR.exists():
        return 0
    pdfs = list(DOWN_DIR.glob("*.pdf"))
    if not pdfs:
        return 0

    print(f"## Phase 0: 00_down 정리 ({len(pdfs)}개 PDF)\n", flush=True)
    folder_name = datetime.now().strftime("%y%m%d-%H%M")
    target_dir = PAPER_DIR / folder_name

    if dry_run:
        print(f"  [DRY] {len(pdfs)}개 PDF → {folder_name}/", flush=True)
        for pdf in pdfs:
    ...
    moved = 0
    for pdf in pdfs:
        safe_name = _re.sub(r'[^\w\s\-.]', '', pdf.name).strip()
        safe_name = _re.sub(r'\s+', '_', safe_name)
        dest = target_dir / safe_name
        try:
            pdf.rename(dest)
            print(f"  → {folder_name}/{safe_name}", flush=True)
            moved += 1
        except Exception as e:
            print(f"  [ERROR] {pdf.name}: {e}", flush=True)
    print(f"\n{moved}개 이동 완료 → {folder_name}/\n", flush=True)
    return moved


def _run_trans_subprocess(command: str, folder_id: str, dry_run: bool, no_gemma: bool = False) -> bool:
    """oopaper_trans.py english/korean subprocess 호출."""
    import subprocess
    if dry_run:
        print(f"  [DRY] {folder_id}: trans {command}", flush=True)
        return True
    trans_script = _SCRIPT_DIR / "oopaper_trans.py"
    cmd = [sys.executable, str(trans_script), command, "--folder", folder_id]
    if no_gemma and command == "korean":
        cmd.append("--no-gemma")
    try:
        result = subprocess.run(cmd, timeout=10800)
        return result.returncode == 0
    except subprocess.TimeoutExpired:
        print(f"  [TIMEOUT] {folder_id}", flush=True)
        return False
    except Exception as e:
        print(f"  [ERROR] {folder_id}: {e}", flush=True)
        return False
...
def _phase3_summary(folder_id: str, dry_run: bool) -> bool:
    ...
    if not eng_files:
        print(f"  [SKIP] {folder_id}: 영문 전문 없음", flush=True)
        return False

    for sf in folder_path.glob("*_00_*서머리.md"):
        if check_file_quality(sf, "summary") is None:
            print(f"  [SKIP] {folder_id}: 서머리 이미 존재", flush=True)
            return False

    if dry_run:
        print(f"  [DRY] {folder_id}: 서머리 생성 예정 (영문 전문 기반)", flush=True)
        return True
    ...
    body = eng_content
    if eng_content.startswith('---'):
        end = eng_content.find('\n---\n', 3)
        if end != -1:
            body = eng_content[end + 5:]

    prompt = (
        f"아래 학술 논문 영문 전문을 읽고 한국어로 서머리 마크다운을 작성하세요.\n"
        f"폴더ID: {folder_id}\n\n"
        "## 출력 형식 (이 형식만 출력, 앞뒤 설명 없이, 본문은 한국어로 작성):\n\n"
        "# [논문 제목 영문 그대로]\n\n"
        "## 개요\n[2~3문장 핵심 요약, 한국어]\n\n"
        "## 핵심 기여\n- [기여1, 한국어]\n- [기여2, 한국어]\n- [기여3, 한국어]\n\n"
        "## 방법론\n[방법론 요약 2~4문장, 한국어]\n\n"
    ...
    try:
        ...
        if result.returncode != 0:
            err_msg = result.stderr[:300] or result.stdout[:300] or f"returncode={result.returncode}"
            print(f"  [ERROR] {folder_id}: {err_msg}", flush=True)
            return False

        parts = eng_files[0].stem.split("_03_", 1)
        name_part = parts[1].replace("_전문(영어)", "") if len(parts) > 1 else "Unknown"

        out_path = folder_path / f"{folder_id}_00_{name_part}_서머리.md"

        # 영문 전문 frontmatter에서 메타 추출
        eng_fm = {}
        if eng_content.startswith("---"):
            end_fm = eng_content.find("\n---\n", 3)
            if end_fm != -1:
                for line in eng_content[3:end_fm].split("\n"):
                    if ":" in line:
                        k, _, v = line.partition(":")
                        eng_fm[k.strip()] = v.strip()

        # summary.md 템플릿에서 frontmatter 블록만 추출해서 조립
        now_str = datetime.now().isoformat()
        tpl = _load_template("summary.md")
        if tpl and tpl.startswith("---"):
            fm_end = tpl.find("\n---\n", 3)
            fm_block = tpl[:fm_end + 5] if fm_end != -1 else ""
        else:
            fm_block = ""
        ...
        if fm_block:
            try:
                fm_filled = fm_block.format(
                    ...
                    translation_ratio="",
                    translation_note="서머리 생성 시점에 한글 번역 미수행 (Phase 4에서 처리)",
                    title="", authors="", year="", doi="", arxiv="",
                    page_count="", abstract="", main_content="",
                    keywords="", tags="", internal_refs="", external_refs="",
                )
            except KeyError:
                fm_filled = ""
        else:
        ...
        final_content = fm_filled + result.stdout.strip()

        out_path.write_text(final_content, encoding='utf-8')
        print(f"  [OK] {folder_id}: 서머리 → {out_path.name}", flush=True)
        return True
    except subprocess.TimeoutExpired:
        print(f"  [TIMEOUT] {folder_id}: 서머리 타임아웃", flush=True)
        return False
    except Exception as e:
        print(f"  [ERROR] {folder_id}: {e}", flush=True)
        return False
...
def _phase6_analysis(folder_id: str, dry_run: bool) -> bool:
    ...
    if list(folder_path.glob("*_05_*분석.md")):
        print(f"  [SKIP] {folder_id}: 분석 파일 이미 존재", flush=True)
        return False

    summary_files = list(folder_path.glob("*_00_*서머리.md"))
    kor_files = list(folder_path.glob("*_04_*전문(한글).md"))
    if not summary_files or not kor_files:
        print(f"  [SKIP] {folder_id}: 서머리 또는 한글 전문 없음", flush=True)
        return False

    if dry_run:
        print(f"  [DRY] {folder_id}: 정밀 분석 예정", flush=True)
        return True
    ...
    kor_content = kor_files[0].read_text(encoding='utf-8')
    body = kor_content
    if kor_content.startswith('---'):
        end = kor_content.find('\n---\n', 3)
        if end != -1:
            body = kor_content[end + 5:]

    prompt = (
        f"아래 논문의 서머리와 한글 번역을 읽고 정밀 분석 리포트를 작성하세요.\n"
        f"폴더ID: {folder_id}\n\n"
        "## 출력 형식 (이 형식만 출력):\n\n"
        "# 정밀 분석: [논문 제목]\n\n"
        "## 1. 연구 배경 및 동기\n[연구가 해결하려는 문제, 기존 연구의 한계]\n\n"
        "## 2. 핵심 아이디어\n[논문의 핵심 아이디어와 직관적 설명]\n\n"
        "## 3. 기술적 상세\n[알고리즘, 아키텍처, 수식 등 기술적 세부 내용]\n\n"
        "## 4. 실험 설계 및 결과 분석\n[실험 설정, 비교 대상, 결과 해석]\n\n"
        "## 5. 강점과 한계\n- 강점:\n- 한계:\n\n"
        "## 6. 실용적 적용 가능성\n[산업/연구 적용 시나리오]\n\n"
        "## 7. 향후 연구 방향\n[논문에서 제시하거나 추론할 수 있는 후속 연구]\n\n"
        "---\n\n"
    ...
    try:
        ...
        if eng_files:
            ...
            name_part = kor_files[0].stem.split("_04_", 1)[-1].replace("_전문(한글)", "")

        out_path = folder_path / f"{folder_id}_05_{name_part}_분석.md"
        out_path.write_text(result.stdout.strip(), encoding='utf-8')
        print(f"  [OK] {folder_id}: 분석 → {out_path.name}", flush=True)
        return True
    except subprocess.TimeoutExpired:
        print(f"  [TIMEOUT] {folder_id}: 분석 타임아웃", flush=True)
        return False
    except Exception as e:
        print(f"  [ERROR] {folder_id}: {e}", flush=True)
        return False
...
def _scan_folders(target_folders):
    """Phase 1 스캔 공통 로직. (all_results, status_counts, needs_*) 반환."""
    all_results = []
    status_counts = {"X": 0, "E": 0, "T": 0, "S": 0, "O": 0}
    needs_english, needs_korean, needs_summary, needs_refs = [], [], [], []
    all_duplicates = []
    ...
    for folder in target_folders:
        ...
        work = result["needs_work"]
        if "english_missing" in work or "english_incomplete" in work:
            needs_english.append(result["folder"])
        if "korean_missing" in work or "korean_incomplete" in work:
            needs_korean.append(result["folder"])
        if "summary_missing" in work or "summary_incomplete" in work:
            needs_summary.append(result["folder"])
        if result["status"] == "S":
            needs_refs.append(result["folder"])

    return all_results, status_counts, needs_english, needs_korean, needs_summary, needs_refs, all_duplicates


def do_run(args):
    """전체 논문 자동 파이프라인 - Phase 0~7 전체 실행."""
    sys.stdout.reconfigure(encoding='utf-8')
    ...
    skip_organize = getattr(args, 'skip_organize', False)
    run_anal = getattr(args, 'anal', False) or phase_filter == 6

    print("# ccpaper run - 완전 자동 파이프라인\n", flush=True)
    print(f"실행 시간: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", flush=True)
    if phase_filter is not None:
        print(f"Phase 필터: {phase_filter}", flush=True)
    if folder_filter:
        print(f"폴더 필터: {folder_filter}", flush=True)
    if dry_run:
        print(f"[DRY-RUN] 실제 변경 없음", flush=True)
    print(f"PRD: doc/d30001_prd.md | TODO: doc/d30004_todo.md\n", flush=True)

    if getattr(args, 'gemma', False):
        os.environ['OOPAPER_USE_GEMMA'] = '1'
        print("[INFO] Gemma 위임 활성화 (Phase 4 번역 초안)\n", flush=True)

    # --- Phase 0: 00_down 정리 ---
    if not skip_organize and (phase_filter is None or phase_filter == 0):
        _phase0_organize(dry_run)
    ...
    if folder_filter:
        target_folders = [f for f in target_folders if f.name == folder_filter]

    total = len(target_folders)
    print(f"총 {total}개 논문 폴더 검사 중...\n", flush=True)

    (all_results, status_counts,
     needs_english, needs_korean, needs_summary, needs_refs,
     all_duplicates) = _scan_folders(target_folders)

    # 상태 통계 출력
    print("### 5단계 상태 (v05)\n", flush=True)
    print("| 상태 | 설명 | 건수 | 비율 |", flush=True)
    print("|:----:|------|:----:|:----:|", flush=True)
    for key, label in [("X","미처리"),("E","영문 추출 완료"),("S","서머리 완료"),("T","번역 완료"),("O","완료")]:
        print(f"| {key} | {label} | {status_counts[key]} | {status_counts[key]*100//max(total,1)}% |", flush=True)
    print(f"| **합계** | | **{total}** | 100% |\n", flush=True)

    print("### 작업 필요 항목\n", flush=True)
    print("| 작업 | 건수 |", flush=True)
    print("|------|:----:|", flush=True)
    print(f"| 영문 추출 | {len(needs_english)} |", flush=True)
    print(f"| 한글 번역 | {len(needs_korean)} |", flush=True)
    print(f"| 서머리 생성 | {len(needs_summary)} |", flush=True)
    print(f"| 참고문헌 매칭 | {len(needs_refs)} |\n", flush=True)

    # 중복 파일 자동 삭제
    if all_duplicates and not dry_run:
        deleted = sum(1 for p in all_duplicates if _safe_unlink(Path(p)))
        if deleted:
            print(f"중복 파일 {deleted}개 삭제\n", flush=True)

    # limit 적용
    needs_english  = needs_english[:limit]
    needs_korean   = needs_korean[:limit]
    needs_summary  = needs_summary[:limit]

    # --- Phase 2: 영문 전문 추출 ---
    if needs_english and (phase_filter is None or phase_filter == 2):
        print(f"## Phase 2: 영문 전문 추출 ({len(needs_english)}건)\n", flush=True)
        ok = fail = 0
        for i, folder_id in enumerate(needs_english, 1):
            print(f"[{i}/{len(needs_english)}] {folder_id}", flush=True)
            if _run_trans_subprocess("english", folder_id, dry_run):
                ok += 1
            else:
                fail += 1
        print(f"\nPhase 2 완료: 성공 {ok} / 실패 {fail}\n", flush=True)

    # --- Phase 3: 서머리 생성 (v05+: 영문 전문 기반, 한글 출력) ---
    if phase_filter is None or phase_filter == 3:
        # Phase 2 직후 → needs_summary 재계산
        if phase_filter is None and needs_english and not dry_run:
            (_, _, _, _, needs_summary, needs_refs, _) = _scan_folders(target_folders)
            needs_summary = needs_summary[:limit]

        if needs_summary:
            print(f"## Phase 3: 서머리 생성 ({len(needs_summary)}건)\n", flush=True)
            ok = fail = 0
            for i, folder_id in enumerate(needs_summary, 1):
                print(f"[{i}/{len(needs_summary)}] {folder_id}", flush=True)
                success = False
                for attempt in range(3):
                    if _phase3_summary(folder_id, dry_run):
                        success = True
                        break
                    if attempt < 2:
                        import time
                        wait = (attempt + 1) * 10
                        print(f"  [RETRY {attempt+1}/2] {folder_id}: {wait}초 후 재시도...", flush=True)
                        time.sleep(wait)
                if success:
                    ok += 1
                else:
                    fail += 1
            print(f"\nPhase 3 완료: 성공 {ok} / 실패 {fail}\n", flush=True)

    # --- Phase 4: 한글 번역 (v05+: 서머리 이후) ---
    if needs_korean and (phase_filter is None or phase_filter == 4):
        print(f"## Phase 4: 한글 번역 ({len(needs_korean)}건)\n", flush=True)
        ok = fail = 0
        for i, folder_id in enumerate(needs_korean, 1):
            print(f"[{i}/{len(needs_korean)}] {folder_id}", flush=True)
            if _run_trans_subprocess("korean", folder_id, dry_run, no_gemma=getattr(args, 'no_gemma', False)):
                ok += 1
            else:
                fail += 1
        print(f"\nPhase 4 완료: 성공 {ok} / 실패 {fail}\n", flush=True)

    # --- Phase 5: 참고문헌 매칭 ---
    if phase_filter is None or phase_filter == 5:
        # Phase 3(서머리) 또는 Phase 4(번역) 직후 → needs_refs 재계산
        if phase_filter is None and (needs_summary or needs_korean) and not dry_run:
            (_, _, _, _, _, needs_refs, _) = _scan_folders(target_folders)

        if needs_refs:
            print(f"## Phase 5: 참고문헌 매칭 ({len(needs_refs)}건)\n", flush=True)
            do_ref_match(argparse.Namespace(folder=folder_filter, dry_run=dry_run))

    # --- Phase 6: 정밀 분석 (--anal 또는 --phase 6 명시 시만) ---
    if run_anal:
        anal_targets = [
            f.name for f in target_folders
            if not list(f.glob("*_05_*분석.md"))
            and list(f.glob("*_00_*서머리.md"))
            and list(f.glob("*_04_*전문(한글).md"))
        ][:limit]
        if anal_targets:
            print(f"## Phase 6: 정밀 분석 ({len(anal_targets)}건)\n", flush=True)
            ok = fail = 0
            for i, folder_id in enumerate(anal_targets, 1):
                print(f"[{i}/{len(anal_targets)}] {folder_id}", flush=True)
                if _phase6_analysis(folder_id, dry_run):
                    ok += 1
                else:
                    fail += 1
            print(f"\nPhase 6 완료: 성공 {ok} / 실패 {fail}\n", flush=True)

    # --- Phase 7: paper_list.md 동기화 ---
    if not dry_run and (phase_filter is None or phase_filter == 7):
        print("\n---\n## Phase 7: paper_list.md 동기화\n", flush=True)
        do_sync_list(argparse.Namespace(dry_run=False))
...
def do_sync_list(args):
    ...
    import re
    sys.stdout.reconfigure(encoding='utf-8')
    print("# ccpaper sync-list - paper_list.md 동기화\n", flush=True)
    ...
    # 2. 실제 폴더 스캔 (strict ^YYMMDD-HHMM$ 만, _attic/ 등 보조 폴더 제외)
    ...
    if not args.dry_run and (status_updates or in_folder_only or in_list_only):
        print("## 업데이트 실행\n", flush=True)
        ...
        # orphan 마킹 (v08+: 폴더 부재 항목 [DELETED YYYY-MM-DD] 태그)
        if in_list_only:
            today = datetime.now().strftime("%Y-%m-%d")
            for folder_id in sorted(in_list_only):
                # 이미 [DELETED 표시 있으면 skip
                entry_text = existing_entries.get(folder_id, "")
                if "[DELETED" in entry_text.split("\n", 1)[0]:
                    continue
                # ### {folder_id} - {title} → ### {folder_id} - [DELETED YYYY-MM-DD] {title}
                content = re.sub(
                    rf'(^###\s+{re.escape(folder_id)}\s*-\s+)(?!\[DELETED)',
                    rf'\g<1>[DELETED {today}] ',
                    content,
                    count=1,
                    flags=re.MULTILINE,
                )
                print(f"- 고아 마킹: `{folder_id}` [DELETED {today}]", flush=True)

        # 헤더 "총 논문수" 카운트 갱신 (v04: 5단계 X/E/T/S/O)
        header_counts = {"X": 0, "E": 0, "T": 0, "S": 0, "O": 0}
        for fid in actual_folders:
            r = check_paper_completeness(PAPER_DIR / fid)
            header_counts[r["status"]] = header_counts.get(r["status"], 0) + 1
        total_n = len(actual_folders)
        new_header = (f"**{total_n}개** (완료: {header_counts['O']}개, "
                      f"서머리: {header_counts['S']}개, 번역: {header_counts['T']}개, "
                      f"영문: {header_counts['E']}개, 미처리: {header_counts['X']}개)")
        content = re.sub(
            r'(##\s*총 논문수\s*\n)\*\*\d+개\*\*[^\n]*',
            rf'\g<1>{new_header}',
            content,
        ...
        print(f"- 헤더 갱신: 총 {total_n}개", flush=True)
...
def do_status(args):
    ...
    if _tpl_status:
        print(_tpl_status.format(
            ...
            e_count=status_counts['E'], e_pct=status_counts['E']*100//max(total,1),
...
def do_fix(args):
    sys.stdout.reconfigure(encoding='utf-8')
    print(f"Running ccpaper fix...", flush=True)
...
def do_clean_duplicates(args):
    """중복 파일만 검사하고 정리합니다."""
    sys.stdout.reconfigure(encoding='utf-8')
    print("# ccpaper clean-duplicates\n", flush=True)
...
def do_ref_update(args):
    ...
    import re
    sys.stdout.reconfigure(encoding='utf-8')
    print("Running ccpaper ref-update...", flush=True)
...
def do_keyword_sync(args):
    ...
    import re
    sys.stdout.reconfigure(encoding='utf-8')
    print("# ccpaper keyword-sync - 키워드 자동 추출 및 동기화\n", flush=True)
    print(f"실행 시간: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n", flush=True)
...
def do_ref_match(args):
    ...
    import re
    sys.stdout.reconfigure(encoding='utf-8')
    print("# ccpaper ref-match - 참고문헌 내부 논문 매칭\n", flush=True)
    print(f"실행 시간: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n", flush=True)
...
def do_quality(args):
    ...
    sys.stdout.reconfigure(encoding='utf-8')
    print("# ccpaper quality - 논문 분석 품질 리포트\n", flush=True)
    print(f"실행 시간: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n", flush=True)
...
def do_download(args):
    ...
    sys.stdout.reconfigure(encoding='utf-8')
    print("# ccpaper download - PDF 자동 다운로드\n", flush=True)
    print(f"실행 시간: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n", flush=True)
    ...
    for list_file in list_files:
        ...
        for item in to_download:
            ...
            try:
                headers = {
                    'User-Agent': 'Mozilla/5.0 (compatible; ccpaper/1.0; academic research)'
                }
    ...
    if total_success > 0:
        print("**→ `ccpaper run` Phase 0으로 다운로드된 PDF를 논문 폴더로 정리하세요**\n", flush=True)
        print("**→ 폴더 이동 후 `ccpaper sync-list` 자동 실행됨 (Phase 7)**\n", flush=True)
...
def main():
    ...
    run_p.add_argument('--phase', type=int, choices=[0,1,2,3,4,5,6,7],
        help="특정 Phase만 실행 (0~7)")
    run_p.add_argument('--anal', action='store_true',
        help="Phase 6 정밀 분석 포함 (기본 제외)")
    ...
    # doctor (종합 정합성 진단, v08+)
    doc_p = subparsers.add_parser('doctor', help="종합 정합성 진단 (folder-id/file-name/multi-pdf/sync/orphan/quality)")
    doc_p.add_argument('--check', choices=['folder-id', 'file-name', 'multi-pdf',
                                           'sync', 'orphan', 'quality'],
                       help="특정 체크만 (생략 시 전체)")
    doc_p.add_argument('--lang', choices=['en', 'ko'], default='en')

    # split-multi (다중 PDF 분리, v08+)
    sm_p = subparsers.add_parser('split-multi', help="다중 PDF 폴더 분리 (1폴더 1논문 정책)")
    sm_p.add_argument('--folder', required=True, help="대상 FOLDER_ID")
    sm_p.add_argument('--lang', choices=['en', 'ko'], default='en')
    sm_p.add_argument('--dry-run', action='store_true')

    # update (sync-list의 양방향 alias, v08+)
    upd_p = subparsers.add_parser('update', help="paper_list 양방향 동기화 (sync-list alias)")
    upd_p.add_argument('--dry-run', action='store_true')
    upd_p.add_argument('--lang', choices=['en', 'ko'], default='en')

    # fix-suffix (suffix 폴더 정규화, v08+)
    fs_p = subparsers.add_parser('fix-suffix', help="suffix 폴더(-NN) 정규화 — 폴더·파일 prefix 일괄 rename")
    fs_g = fs_p.add_mutually_exclusive_group(required=True)
    fs_g.add_argument('--folder', help="특정 suffix 폴더만")
    fs_g.add_argument('--batch', action='store_true', help="모든 suffix 폴더 일괄")
    fs_p.add_argument('--lang', choices=['en', 'ko'], default='en')
    fs_p.add_argument('--dry-run', action='store_true')
    fs_p.add_argument('--use-mtime', action='store_true',
    ...
    if args.command == 'run' or args.command is None:
        ...
        # 별도 스크립트 위임 (oopaper_doctor.py)
        from oopaper_doctor import cmd_doctor as _doctor
        rc = _doctor(args)
        sys.exit(rc or 0)
    elif args.command == 'split-multi':
        # 별도 스크립트 위임 (oopaper_split_multi.py)
        from oopaper_split_multi import cmd_split as _split
        rc = _split(args)
        sys.exit(rc or 0)
    elif args.command == 'update':
        # update = sync-list의 alias (양방향 동기화)
        do_sync_list(args)
    elif args.command == 'fix-suffix':
        # 별도 스크립트 위임 (oopaper_fix_suffix.py)
        from oopaper_fix_suffix import cmd_fix_suffix as _fix_suffix
        rc = _fix_suffix(args)
        sys.exit(rc or 0)
    else:
