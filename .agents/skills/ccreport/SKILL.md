---
name: ccreport
description: "공통: `.codex/guides/common_guide.md` | 가이드: `.agents/skills/ccreport/references/guide.md` | Word: `.agents/skills/ccword/SKILL.md`"
---

<!-- ccporting:generated-from-upstream -->
<!-- 원본 스킬은 upstream/ 폴더에 보관된다. -->

> 공통: `.codex/guides/common_guide.md` | 가이드: `.agents/skills/ccreport/references/guide.md` | Word: `.agents/skills/ccword/SKILL.md`

데이터 소스 기반 리포트 생성. 템플릿 렌더링, 다중 포맷(MD/PDF/PPTX) 지원.

> **Word 문서 생성**: `.agents/skills/ccword/SKILL.md` 스킬 참조 (word, quotation 기능 분리됨)
...
| **하지 않는 것** | Word 문서(→ccword), 학술 논문(→ccsota), PPT 노트(→ccppt) |
| **참조 범위** | 현재 프로젝트 내부 데이터 파일·템플릿 / 외부 데이터 서비스 자동 연동 안 함 |
| **수정 대상** | 리포트 출력 파일 (MD/PDF/PPTX) |
| **실행 레벨** | [반자동] — 데이터 소스·포맷 확인 후 생성 |
| **에이전트 호환** | Codex 권장 — `uv run` 기반 변환 라이브러리 자동 실행 / 다른 에이전트: pandoc·weasyprint 수동 실행 |
...
| ~~`review`~~ | **`ccsota review`로 이관됨**. 논문 교정은 `ccsota review` 사용 (paper_quality_checklist.md 통합) |
...
> **필수**: `.agents/skills/ccreport/templates/ooreport_algorithm.md` 템플릿 구조를 따를 것
...
| 템플릿 | `.agents/skills/ccreport/templates/ooreport_*.md` |
| 스크립트 | `.agents/skills/ccreport/scripts/ooreport_*.py` |
| 가이드 | `.agents/skills/ccreport/references/guide.md` |
