...
| `cccommit help` | 서브명령어 목록 표시 |

| `cccommit version` | 스킬 버전 정보 (v03) |

| `cccommit status` | 서브명령어 리스트, 현재 상태 |

| `cccommit check` | references/checklist.md 기반 체크 및 리포팅 | 터미널 |

| `cccommit run` | Git 커밋 + 기본 push 실행 |

| `cccommit show checklist` | 역할 수행 체크리스트 표시 | 터미널 |

| `cccommit add checklist "항목"` | 체크리스트 항목 추가 | checklist.md |
...
### 2.1 [DOC_ACTION_NEEDED] 처리 프로토콜

`cccommit run` 출력에 `[DOC_ACTION_NEEDED]`가 있으면 커밋이 블록된 것이다.
...
cccommit github                    # 전체 태스크 변환

cccommit github --task-id 1,2,3    # 특정 태스크만

cccommit github --dry-run          # 미리보기

cccommit github --sync             # 기존 이슈와 동기화
...
| 커밋 + push + 이력 정리 | `cccommit run` | - |

| 페이즈 완료 배포 | `cccommit run` 후 추가 배포 | `/gsd:ship` |

| PR 브랜치 생성 | `cccommit github` | `/gsd:pr-branch` |

| 마일스톤 완료 처리 | `cccommit run` → 수동 | `/gsd:complete-milestone` |
...
- `cccommit run` → git commit + 기본 push + d{SP}0004 완료항목 d{SP}0010 이동, Conventional Commits 형식
...
cccommit run               # 커밋 + push + OAIS 이력 정리(d0010)
