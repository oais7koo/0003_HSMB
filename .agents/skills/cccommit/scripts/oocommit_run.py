...
명령어:
    cccommit run          커밋 + 이력 정리 통합 실행 (기본)
    cccommit commit       Git 커밋만 수행
    cccommit sync         이력 정리만 수행 (d0004 -> d0010)
    cccommit preview      변경사항 및 이동 대상 미리보기
"""
...
# scripts/ -> cccommit/ -> skills/ -> .codex/ -> PROJECT_ROOT
...
def step_pre_commit_doc_check(all_files: list, options: dict) -> int:
    """커밋 전 상세문서 업데이트 단계.
    반환: 0=계속, 1=NEED_ACTION(Claude가 처리 후 cccommit commit으로 재실행)
    """
    ...
    if feature_docs:
        ...
        for i, doc in enumerate(feature_docs, 1):
            ...
            print(f"  {i}. ccf update {doc_id} --sp {inferred_sp}")
        print(f"  n. ccf new --sp {inferred_sp}  (새 상세문서 생성)")
        print(f"  s. 스킵  → cccommit commit 으로 문서 없이 커밋")
    else:
        print(f"  - {sp_label} 상세문서 없음")
        print()
        print("[DOC_ACTION_NEEDED]")
        print(f"  SP={inferred_sp} | 상세문서가 없습니다:")
        print(f"  y. ccf new --sp {inferred_sp}  (새 상세문서 생성 후 커밋)")
        print(f"  s. 스킵  → cccommit commit 으로 문서 없이 커밋")
...
def print_usage():
    ...
    print("cccommit - Git 커밋 및 이력 정리")
    print()
    print("사용법:")
    print("    cccommit status       Git 상태 및 변경사항 요약")
    print("    cccommit run          커밋 + 이력 정리 통합 실행")
    print("    cccommit commit       Git 커밋만 수행")
    print("    cccommit sync         이력 정리만 수행 (d0004 -> d0010)")
    print("    cccommit preview      변경사항 및 이동 대상 미리보기")
    print("    cccommit clear        오래된 커밋 squash + .git 용량 정리")
    print()
    print("옵션:")
    ...
    print()
    print("예시:")
    print("    python .agents/skills/cccommit/scripts/oocommit_run.py run")
    print(
        '    python .agents/skills/cccommit/scripts/oocommit_run.py commit --message "fix: 버그 수정"'
    )
    print("    python .agents/skills/cccommit/scripts/oocommit_run.py preview")
...
def cmd_status():
    ...
    print("\n### 다음 단계\n")
    if total > 0:
        print("커밋이 필요하면 `cccommit run` 또는 `cccommit preview` 를 실행하세요.")
    else:
...
def _mark_completed_todo_details_in_commit():
    ...
    try:
        ...
        ootodo_path = Path(__file__).parent.parent.parent / "cctodo" / "scripts" / "ootodo_run.py"
...
def main():
    ...
    if cmd == "status":
        ...
        print(f"[check] cccommit 체크리스트 안내")
        _print_skill_help("cccommit")
        return 0
    elif cmd in ("help", "-h"):
        _print_skill_help("cccommit")
        return 0
