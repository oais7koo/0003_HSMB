...
CACHE_FILE = Path.home() / ".claude" / "plugins" / "oh-my-claudecode" / ".usage-cache.json"
...
def run():
    ...
    # 모델 정보 읽기
    model_cache = Path.home() / ".claude" / ".omc" / "model-cache.json"
    model_name = "unknown"
...
def main():
    ...
    if cmd in ("", "status"):
        ...
        print("ootoken 버전: v01")
    elif cmd in ("help", "-h", "--help"):
        _print_skill_help("ootoken")
    else:
