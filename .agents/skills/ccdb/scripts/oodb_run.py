...
명령어:
    ccdb status           DB 상태 및 미해결 이슈 요약
    ccdb run              Phase 1 + Phase 2 실행 (분석 → 기록 → 전체 수정 → 문서 업데이트)
    ccdb optimize         run + 최적화 (인덱스, 쿼리, 정규화)
    ccdb doc              문서화만 수행 (d0006_db.md 업데이트)
"""
...
def print_usage():
    ...
    print("ccdb - DB 수정 및 최적화 워크플로우")
    print()
    print("사용법:")
    print("    ccdb status           DB 상태 및 미해결 이슈 요약")
    print("    ccdb run              Phase 1 + Phase 2 실행 (분석 → 기록 → 수정 → 문서)")
    print("    ccdb optimize         run + 최적화 (인덱스, 쿼리, 정규화)")
    print("    ccdb doc              문서화만 수행 (d0006_db.md 업데이트)")
    print()
    print("옵션:")
...
def cmd_status(db_path, options):
    ...
    print("| 명령어 | 설명 |")
    print("|--------|------|")
    print("| `ccdb status` | 서브명령어 리스트, DB 상태/미해결 이슈 |")
    print("| `ccdb run` | validate + 분석 + 수정 (3-Phase 통합) |")
    print("| `ccdb validate` | SQL 쿼리 스키마 검증만 (EXPLAIN) |")
    print("| `ccdb optimize` | run + 최적화 |")
    print("| `ccdb doc` | d0006_db.md 문서화 |")
    print()
    ...
    print("---")
    print("[TIP] 'ccdb run'으로 문제점 수정을 시작하세요.")
...
def cmd_optimize(db_path, options):
    ...
    # 4. 가이드
    print("## 최적화 가이드\n")
    print("  [INFO] ccdb optimize는 서브에이전트 기반으로 동작합니다.")
...
def main():
    ...
    if cmd == "status":
        ...
        if options.get("fix") or "--fix" in args:
            return cmd_optimize(db_path, options)
        print(f"[check] ccdb 체크리스트 안내 (--fix 로 optimize 수행)")
        _print_skill_help("ccdb")
        return 0
    elif cmd == "doc":
