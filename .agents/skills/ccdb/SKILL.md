...
| 구분 | 문서 | 용도 |
|------|------|------|
| `ccdb help` | 서브명령어 목록 표시 | 터미널 |
| `ccdb version` | 스킬 버전 정보 (v04) |
...
| 명령어 | 설명 |
|--------|------|
| `ccdb status` | 서브명령어 리스트, DB 상태/미해결 이슈 |
| `ccdb check` | references/checklist.md 기반 체크 및 리포팅 | 터미널 |
| `ccdb show checklist` | 역할 수행 체크리스트 표시 | 터미널 |
| `ccdb add checklist "항목"` | 체크리스트 항목 추가 | checklist.md |
| `ccdb run` | **check + 분석 + 수정** (3-Phase 통합) |
| **`ccdb run this`** | **직전 작업 DB 분석** (→ common_guide.md §9) |
| `ccdb check` | SQL 쿼리 스키마 검증만 (EXPLAIN) |
| `ccdb check --fix` | run + 최적화 |
| `ccdb doc` | d0006_db.md 문서화 |
| `ccdb design` | **DB 설계**: PRD/Plan 기반 테이블/컬럼 설계 |
| `ccdb dev` | **DB 개발**: 설계된 스키마 마이그레이션 (백업 필수) |
...
| 보관 | 최근 5개 유지 (자동 정리) |
| 복원 | `ccdb restore --backup [filename]` |
