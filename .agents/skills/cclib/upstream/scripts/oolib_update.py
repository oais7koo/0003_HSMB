...
명령어:
    oolib run              전체 모듈 분석
    oolib run [모듈명]     특정 모듈 분석 (예: oolib run image)
    oolib update           d0005_lib.md 문서 업데이트 (oais 모듈 + 함수 상세)
...
def print_usage():
    ...
    print("oolib - oais 모듈 분석 및 문서화")
    print()
    print("사용법:")
    print("    oolib run              전체 모듈 분석")
    print("    oolib run [모듈명]     특정 모듈 분석 (예: image, metrics)")
    print("    oolib update           d0005_lib.md 문서 업데이트 (oais 모듈 + 함수 상세)")
    print()
    print("옵션:")
    ...
    print("    python .claude/skills/oolib/scripts/oolib_update.py run image")
...
def build_lib_doc():
    ...
    history = [f"- {new_ver} {today} — oolib doc 자동 현행화 (oais v{oais_ver})"]
