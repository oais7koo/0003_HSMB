...
## 문서 이력 관리
- v03 2026-05-17 — oais 모듈 스킬로 정체성 명확화: d0005=oais 모듈 문서(§1 모듈+§2 함수 상세·출처 추적), 출처 docstring 태그 규약 추가, 의존성 제외→d0009, oo/→oais/ 정정
...
| 항목 | 내용 |
|------|------|
| **핵심 역할** | 프로젝트 루트 `oais/` 모듈의 정합화·정리·문서화 |
| **하는 것** | oais 모듈 분석, 이슈 d{SP}0004 등록·해결, d0005_lib.md 모듈 문서화 |
| **하지 않는 것** | 서브프로젝트 코드 수정(→ccfix), DB 수정(→ccdb), 성능 최적화(→ccopti), 의존성 관리(→ccuv) |
| **참조 범위** | 현재 프로젝트 내부 `oais/` 모듈만 / pyproject 의존성·외부 라이브러리 자동 포함 안 함 |
| **수정 대상** | `oais/*.py`, `d{SP}0004_todo.md`, `d{SP}0010_history.md`, `d0005_lib.md` |
| **실행 레벨** | [자동] — Phase 1(분석) → Phase 2(수정) 자동 실행 |
| **에이전트 호환** | Codex 권장 — `uv run` 환경 자동 사용 / 다른 에이전트: 분석·수정 단계를 수동 실행 |

## 1. 개요

프로젝트 루트 `oais/` 모듈의 정합화·정리 워크플로우 + d0005 모듈 문서화.
...
| 명령어 | 설명 |
|--------|------|
| `cclib help` | 서브명령어 목록 표시 |
| `cclib version` | 스킬 버전 정보 (v01) |
| `cclib status` | 서브명령어 리스트, 상태/미해결 이슈 |
| `cclib check` | references/checklist.md 기반 체크 및 리포팅 | 터미널 |
| `cclib show checklist` | 역할 수행 체크리스트 표시 | 터미널 |
| `cclib add checklist "항목"` | 체크리스트 항목 추가 | checklist.md |
| `cclib run` | Phase 1+2 (분석->수정->문서) |
| **`cclib run this`** | **직전 작업 모듈 최적화** (→ common_guide.md §9) |
| `cclib check --fix` | run + 최적화 (구 `optimize`) |
| `cclib doc` | d0005_lib.md 문서화만 (oais 모듈 구조) |
| `cclib cleanup [대상]` | 코드/import/파일 정리 (cleanup 흡수) |

옵션: `--module [name]`, `--dry-run`, `--interactive`, `--report`
cleanup 옵션: `--type code\|imports\|files\|all`, `--safe`, `--aggressive`

실행: `uv run python .agents/skills/cclib/scripts/oolib_run.py [args]`

> **d0005_lib.md 범위**: `oais/` 패키지의 모듈 구조(§1) + **함수 상세**(§2: 함수·시그니처·설명·출처)를 기록한다. `출처`는 함수화 전 SP02(`02_pycode/`)에서 개발·검증된 원본 파일 — docstring `출처:` 태그 자동 추출 + 수동 기재 보존(하이브리드). Python 의존성(pyproject.toml)은 d0009(ooenv) 관할 — 포함하지 않는다. 상세 규약: `references/guide.md §5`.
...
- 예: `| L001 | 2026-01-02 | [FIX] oais/utils.py - 중복 | 높음 | 대기 |`
...
`cclib doc --compact` 또는 `ccdoc run --compact` 호출 시 적용. guide.md 템플릿보다 우선:
...
> **관련 문서**: `.codex/guides/debugging_guide.md` | `00_doc/d{SP}0004_todo.md` | `00_doc/sp00/d0005_lib.md`
