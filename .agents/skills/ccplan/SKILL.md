---
name: ccplan
description: "공통: .claude/guides/common_guide.md | 상세 가이드: .claude/skills/ccplan/references/guide.md"
---

<!-- ccporting:generated-from-upstream -->
<!-- 원본 Claude 스킬은 upstream/ 폴더에 보관된다. -->

> 공통: .claude/guides/common_guide.md | 상세 가이드: .claude/skills/ccplan/references/guide.md
...
| Do/Don't | Plan·아키텍처 / PRD(→ooprd)·구현(→oodev)·이슈(→oofix). 컨텍스트: `--sp N` 또는 `oocontext N` |
...
- 필터: PRD 5.1 진행=아니오 제외. check: 커버리지 100%·순환의존성 없음·크리티컬 패스 최소·스프린트 균형. 완료 후 `oofeature needed` 권장
...
`sync`/`run` 시 `00_doc/sp{N}/` 스캔 → 8.2절 자동 갱신. 패턴 `*_상세{기획,설계,개발,검증}_*.md`. 단계 ⚪기획→🔵설계→🟡개발→🟢검증→✅완료. 연동 `oofeature`/`oodev run dXXXX`. 형식: references/guide.md §4.6

## 9. 도구 연동 / 딥러닝 실험

- oodev: detail→상세 설계→TDD | Task Master: Epic→Task / Feature→Subtask / 스프린트→태그 / 의존성→dependencies
...
- 프레임워크 감지 → `.claude/reference/development/{framework}/`: FastAPI→`fast-api/`, Streamlit→`references/streamlit_guide.md`
- 입력 `d{SP}0001_prd.md` / 출력 `d{SP}0002_plan.md` / 관련 `oodev/SKILL.md`·`sc/task.md`·`sc/estimate.md`
- GSD: `ccplan run`↔`/gsd:plan-phase [N]`(옵션 `--research`), `ooprd clarify`↔`/gsd:discuss-phase [N]`, `ccplan check --fix`↔`/gsd:plan-milestone-gaps`. 차이: OAIS는 `d{SP}0002_plan.md` / GSD는 `.planning/phases/N/PLAN.md`. 조합: references/guide.md §4.9
...
> Karpathy 코딩 4원칙 준수: `.claude/rules/karpathy-guidelines.md` (Think Before / Simplicity / Surgical / Goal-Driven)
