...
명령어:
    oouser run              d0008_user.md 신규 생성 (기본)
    oouser status           d0008_user.md 현황 조회
    oouser add [기능명]     특정 기능 사용법 추가
    oouser faq [질문]       FAQ 항목 추가
    oouser update           전체 문서 현행화
    oouser sync             PRD 기반 기능 목록 동기화
"""
...
def print_usage():
    ...
    print("oouser - 사용자 가이드 문서 관리")
    print()
    print("사용법:")
    print("    oouser run              d0008_user.md 신규 생성 (기본)")
    print("    oouser status           d0008_user.md 현황 조회")
    print("    oouser add [기능명]     특정 기능 사용법 추가")
    print("    oouser faq [질문]       FAQ 항목 추가")
    print("    oouser update           전체 문서 현행화")
    print("    oouser sync             PRD 기반 기능 목록 동기화")
    print()
    print("예시:")
    print("    python .claude/skills/oouser/scripts/oouser_run.py run")
    print("    python .claude/skills/oouser/scripts/oouser_run.py add \"로그인\"")
    print("    python .claude/skills/oouser/scripts/oouser_run.py faq \"비밀번호 분실\"")
    print("    python .claude/skills/oouser/scripts/oouser_run.py sync")
...
def cmd_add(args):
    ...
    if not USER_FILE.exists():
        print(f"[ERROR] {USER_FILE}가 없습니다.")
        print("[TIP] oouser run 으로 먼저 생성하세요.")
        return 1
...
def cmd_update():
    ...
    if body_before == body_after:
        ...
        if match:
            ...
            new_row = f"\n| {new_version} | {today} | 문서 현행화 (oouser) |"
    ...
    if USER_FILE.exists():
        ...
        if missing:
            ...
            for m in missing:
                print(f"  - {m['name']}")
            print()
            print("[TIP] oouser add \"기능명\" 으로 추가하세요.")
        else:
