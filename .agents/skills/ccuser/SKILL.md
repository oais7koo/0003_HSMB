---
name: ccuser
description: "공통: `.claude/guides/common_guide.md` 참조"
---

<!-- ccporting:generated-from-upstream -->
<!-- 원본 Claude 스킬은 upstream/ 폴더에 보관된다. -->

# ccuser - 사용자 가이드 관리 스킬
...
| 명령어 | 설명 | 출력 |
|--------|------|------|
| `ccuser help` | 서브명령어 목록 표시 | 터미널 |
| `ccuser version` | 스킬 버전 정보 (v01) | 터미널 |
| `ccuser run` | 신규 생성 | 00_doc/d{SP}0008_user.md |
| `ccuser update` | 현행화 — 현재 기능 스캔 → 사용자 가이드 갱신 | d{SP}0008_user.md |
| `ccuser update --dry-run` | 변경 예정 내용 미리 출력 (실제 수정 안 함) | 터미널 |
| `ccuser status` | 서브명령어 리스트, 현황 조회 | 터미널 |
| `ccuser check` | references/checklist.md 기반 체크 및 리포팅 | 터미널 |
| `ccuser show checklist` | 역할 수행 체크리스트 표시 | 터미널 |
| `ccuser add checklist "항목"` | 체크리스트 항목 추가 | checklist.md |
| `ccuser add [기능명]` | 기능 사용법 추가 | 00_doc/d{SP}0008_user.md |
| `ccuser faq [질문]` | FAQ 추가 | 00_doc/d{SP}0008_user.md |
| `ccuser sync` | PRD 기반 동기화 | 00_doc/d{SP}0008_user.md |

실행: `uv run python .claude/skills/ccuser/scripts/oouser_run.py [args]`
...
`ccuser run --compact` 또는 `oodoc run --compact` 호출 시 적용. guide.md 템플릿보다 우선:
