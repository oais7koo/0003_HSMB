    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            ...
            if node.module:
                ...
                if node.level > 0:
                     # For simplicity in this script, we just treat .module as oo.module
                     # This is heuristic but identifying the cycle is usually enough
                     imports.append(f"oo.{node.module}" if node.module else "oo")
                else:
...
def main():
    ...
    oo_dir = os.path.join(root_dir, 'oo')

    print(f"Scanning {oo_dir}...")

    py_files = {}
    for root, _, files in os.walk(oo_dir):
        for file in files:
            if file.endswith('.py'):
                full_path = os.path.join(root, file)
                # Module name construction: oo.submodule
    ...
    for module_name, file_path in py_files.items():
        ...
        for imp in raw_imports:
            # Normalize import
            target = imp
            if target.startswith('oo'):
                # We only care about imports within 'oo' package
                # Check if it maps to a known file
                # Exact match
                if target in py_files and target != module_name:
                    graph[module_name].add(target)
                else:
                    # Check if it's a parent package import (e.g. oo.utils.string -> oo.utils)
                    parts = target.split('.')
                    while len(parts) > 1:
                        parent = '.'.join(parts)
