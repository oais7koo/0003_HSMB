...
- v04 2026-05-07 — `oocommit run` 커밋 전 상세문서(oof) 업데이트 단계 추가 (항상 실행, 완료 후 oocommit commit으로 진행)
...
`oocommit run` 출력에 `[DOC_ACTION_NEEDED]`가 있으면 커밋이 블록된 것이다.

**상세문서 있을 때 (ACTION=SELECT)**:
1. 출력된 목록을 사용자에게 보여주며 번호 선택 요청
2. 사용자 선택 → `oof update <doc_id> --sp N` 실행
3. 성공 → `oocommit commit` 으로 커밋 진행
4. 실패 → 커밋 중단 (오류 보고)

**상세문서 없을 때 (ACTION=NEW)**:
1. "상세문서가 없습니다. 새로 만드시겠어요?" 확인
2. y → `oof new --sp N` → 성공 시 `oocommit commit`
3. s → `oocommit commit` (문서 없이 커밋)
4. oof 실패 → 커밋 중단

> oof update/new 완료 후에는 `oocommit commit`을 사용한다 (run은 step 0을 다시 실행하므로).
