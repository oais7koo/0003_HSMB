#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""oodoc clear - 문서 이력 테이블 정리 (5개 초과 행 제거)
...
사용법:
    uv run python .claude/skills/oodoc/scripts/oodoc_clear.py
    uv run python .claude/skills/oodoc/scripts/oodoc_clear.py --keep 3
    uv run python .claude/skills/oodoc/scripts/oodoc_clear.py --dry-run
    uv run python .claude/skills/oodoc/scripts/oodoc_clear.py --scope 00_doc
    uv run python .claude/skills/oodoc/scripts/oodoc_clear.py --scope skills
    uv run python .claude/skills/oodoc/scripts/oodoc_clear.py --scope guides
"""
...
# 경로 설정: scripts/ → oodoc/ → skills/ → .claude/ → PROJECT_ROOT/
...
def run_clear(keep: int = 5, dry_run: bool = False, scope: str = "all") -> dict:
    ...
    print(f"[oodoc clear] {mode_label}이력 정리 시작")
...
def main():
    parser = argparse.ArgumentParser(
        description="oodoc clear - 문서 이력 테이블 정리"
    )
    parser.add_argument(
