#!/usr/bin/env python
"""
oopaper_auto.py - oopaper 자동화 헬퍼 (v04+)
...
실행 예:
  uv run python .claude/skills/oopaper/scripts/oopaper_auto.py organize --dry-run
  uv run python .claude/skills/oopaper/scripts/oopaper_auto.py dedup
  uv run python .claude/skills/oopaper/scripts/oopaper_auto.py meta --folder 260428-0001
"""
import argparse
import re
import shutil
...
PROJECT_ROOT = _HERE.parents[4]  # scripts → oopaper → skills → .claude → root
...
def cmd_organize(args):
    ...
    print(f"# oopaper organize - 00_down 자동 정리\n")
    ...
    if moved:
        print(f"\n**→ `oopaper sync-list` 실행하여 paper_list.md 동기화하세요**")
...
def cmd_dedup(args):
    """폴더 간 동일 논문(중복) 검출."""
    print(f"# oopaper dedup - 폴더 간 중복 검출\n")
    print(f"실행 시간: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
...
def cmd_meta(args):
    ...
    print(f"# oopaper meta - PDF 메타데이터 추출\n")
...
def cmd_ref_match_fuzzy(args):
    ...
    print(f"# oopaper ref-match-fuzzy\n")
