...
def add_formatted_text(paragraph, text, default_font='Malgun Gothic', default_size=Pt(11)):
    # Handle <br> as in-paragraph line break (split, recurse on parts)
    if re.search(r'<br\s*/?>', text):
        br_parts = re.split(r'<br\s*/?>', text)
        for idx, part in enumerate(br_parts):
            if idx > 0:
                paragraph.add_run().add_break()
            add_formatted_text(paragraph, part, default_font, default_size)
        return
    pattern = r'(\*\*(.+?)\*\*|\*(.+?)\*|`(.+?)`)'
    last_end = 0
    ...
    for i, row_data in enumerate(parsed_rows):
        ...
        for j, cell_text in enumerate(row_data):
            if j < num_cols:
                ...
                # Use add_formatted_text — handles **bold**, *italic*, `code`, <br>
                ...
                # Default font for runs that don't already have a font set
                # (preserves Consolas font for `code` parts from add_formatted_text)
                for para in cell.paragraphs:
                    for run in para.runs:
...
def convert_md_to_docx(input_path: str, output_path: str):
    ...
    for line in lines:
        ...
        if line.startswith('# '):
            ...
            # Bullet list (handles nested with leading spaces, - * + markers)
            m = re.match(r'^(\s*)[-*+] (.*)$', line)
