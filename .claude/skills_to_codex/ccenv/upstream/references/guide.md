...
```json
{
  ...
  "enabledMcpjsonServers": ["sequential-thinking", "desktop-commander"]
}
```
...
### 3.9 Python 개발 환경 권한 적용 (선택)

> Python 개발(pytest/black/isort/mypy/django/flask 등) 권한이 필요한 경우, `references/settings_python_dev.json`을 `.claude/settings.json`에 머지하여 사용한다. 기본 `settings.json`은 디자인/펜슬 위주의 최소 권한이다.

#### 위치

```
.claude/skills/ooenv/references/settings_python_dev.json
```
...
| 카테고리 | 권한 |
|---------|------|
| Python 코어 | `Bash(python:*)`, `Bash(pip:*)` |
| 테스트·린트 | `Bash(pytest:*)`, `Bash(black:*)`, `Bash(isort:*)`, `Bash(flake8:*)`, `Bash(mypy:*)` |
| 웹 프레임워크 | `Bash(django-admin:*)`, `Bash(flask:*)`, `Bash(uvicorn:*)`, `Bash(gunicorn:*)` |
| Git | `Bash(git:*)` |
| 모드 | `defaultMode: allowEdits` (기본 `acceptEdits`보다 자율도 ↑) |

#### 적용 방법

1. **머지 (권장)**: `references/settings_python_dev.json`의 `permissions.allow` 항목을 현재 `settings.json`의 `allow`에 추가
2. **교체**: `settings.json`을 백업 후 `settings_python_dev.json` 내용으로 덮어쓰기 (디자인 권한 손실 주의)
3. **모드만 변경**: `defaultMode`만 `allowEdits`로 변경 (자율도 ↑가 필요한 경우)

> **머지 시 유의**: `defaultMode`, `env`, `hooks` 등 비권한 영역은 `settings.json` 기존 값 유지. `permissions.allow`만 합집합으로 확장.

#### 회수 방법

추가했던 Python 권한을 다시 제거하려면 `settings.json`의 `allow`에서 해당 항목 삭제. 원본 보존 위해 `settings_python_dev.json`은 그대로 둔다.

---

### 3.10 중복 도구 정책 — CLI 우선 시 MCP 서버 제거

> 동일 외부 서비스(예: GitHub)에 대해 CLI와 MCP 서버가 둘 다 가용하면 **CLI 우선** 정책을 적용한다. MCP 서버는 제거하여 인증 이중 관리·plugin doctor 경고를 회피한다.

#### 적용 사례 (현재 정책)

| 외부 서비스 | CLI 도구 | 중복 MCP/Plugin | 권장 |
|------------|---------|-----------------|------|
| **GitHub** | `gh` (DEV_TOOLS 필수) | `github@claude-plugins-official` plugin (MCP 서버 자동 spawn) | gh CLI 인증 시 plugin 제거 |

#### 판정 룰

1. CLI 인증 상태 확인 — 예: `gh auth status` 통과
2. 동일 서비스 MCP 서버 또는 plugin이 settings에 등록되어 있는지 확인
3. 둘 다 있으면 → MCP/plugin 제거 (CLI 우선)

#### 제거 절차 (예: github)

```
.claude/settings.json `enabledPlugins`:
- "github@claude-plugins-official": true  ← 라인 삭제

.claude/settings.local.json `permissions.allow`:
- "mcp__github__create_repository"  ← 라인 삭제 (잔재 권한)
```
...
| 효과 | 내용 |
|------|------|
| `/doctor` 경고 해소 | 더 이상 등록된 plugin이 아니므로 검증 대상 외 |
| 인증 단일화 | gh CLI keyring만 관리 (PAT env 별도 관리 불필요) |
| 기능 손실 | 없음 — gh CLI가 모든 GitHub 작업(issue, pr, repo, api) 커버 |

> **예외**: Claude가 MCP의 구조화된 응답을 적극 활용하는 워크플로우(예: 자동화 스크립트가 MCP 도구를 호출)는 plugin 유지. 일반 git 협업·oocommit 흐름은 gh CLI로 충분.
...
| .claude/skills/ooenv/references/settings_python_dev.json | Python 개발 환경 권한 프로필 (§3.9) |
