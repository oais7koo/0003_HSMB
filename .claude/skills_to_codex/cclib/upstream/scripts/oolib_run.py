...
명령어:
    oolib run              현재 문제점 수정 (d0004 이슈 기반)
    oolib optimize         문제점 수정 + 최적화
    oolib status           상태 및 이슈 요약
    oolib doc              문서화만 수행 (d0005_lib.md)
"""
...
def print_usage():
    ...
    print("oolib - oo 모듈 수정 및 최적화")
    print()
    print("사용법:")
    print("    oolib run              현재 문제점 수정 (d0004 이슈 기반)")
    print("    oolib optimize         문제점 수정 + 최적화")
    print("    oolib status           상태 및 이슈 요약")
    print("    oolib doc              문서화만 수행 (d0005_lib.md)")
    print()
    print("옵션:")
    ...
    print()
    print("예시:")
    print("    uv run python .claude/skills/oolib/scripts/oolib_run.py run")
    print("    uv run python .claude/skills/oolib/scripts/oolib_run.py optimize --module config")
    print("    uv run python .claude/skills/oolib/scripts/oolib_run.py status")
...
def cmd_status(options):
    ...
    print()
    print("---")
    print("[TIP] 'oolib run'으로 문제점 수정을 시작하세요.")
...
def cmd_optimize(options):
    ...
    # 5. 가이드
    print("## 최적화 가이드\n")
    print("  [INFO] oolib optimize는 서브에이전트 기반으로 동작합니다.")
...
def main():
    ...
    if cmd == "run":
        ...
        if "--fix" in args:
            return cmd_optimize(options)
        print(f"[check] oolib 체크리스트 안내 (--fix 로 optimize 수행)")
