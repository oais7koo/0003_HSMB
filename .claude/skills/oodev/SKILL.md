---
name: oodev
description: "TDD 기반 개발 스킬 'oodev', 'TDD', '개발', '구현', 'GREEN', 'REFACTOR' 등의 키워드로 트리거된다"
metadata:
  version: v09
  category: core-dev
---
...
> 상세 가이드: .claude/skills/oodev/references/guide.md
...
- v02 2026-04-06 — 상세기획 문서 지정 개발 추가: `oodev run dXXXX`
...
| 스킬 | 역할 | 문서 |
|------|------|------|
| `oodev help` | 서브명령어 목록 표시 | 터미널 |
| `oodev version` | 스킬 버전 정보 (v07) |
| ooplan | 설계 | d{SP}0002_plan.md |
| oodev | **d{SP}0003 생성 (INIT)** + **GREEN + REFACTOR** | d{SP}0002_plan.md, d{SP}0003_test.md |
...
| 명령어 | 설명 |
|--------|------|
| oodev init --framework NAME [경로] | 프레임워크 템플릿 기반 개발서 생성 |
| `oodev status` | 서브명령어 리스트, 진행 상태 |
| `oodev check` | references/checklist.md 기반 체크 및 리포팅 | 터미널 |
| oodev show checklist | 역할 수행 체크리스트 표시 |
| oodev add checklist "항목" | 체크리스트 항목 추가 |
| oodev run | 미완료 Feature 전체 구현 |
| oodev run [ID] | 특정 Feature 구현 (F002-1 등) |
| oodev run dXXXX | 상세기획 문서(dXXXX_*.md) 기반 개발 |
| oodev run dXXXX [ID] | 상세기획 문서 기반 특정 Feature 구현 |
| **oodev run this** | **직전 작업 Feature/파일 계속 구현** (→ common_guide.md §9) |
| oodev queue | 대기 큐 처리 |
| oodev check --fix | 코드 최적화 (구 `optimize`) |
...
| 단계 | 담당 | 작업 | 산출물 |
|------|------|------|--------|
| INIT | **oodev** | 테스트 문서 생성 | d{SP}0003_test.md (첫 실행 시) |
| RED | **ootest** | TC 코드 작성 + 실패 확인 | tests/TC*.py, d{SP}0003 Part C |
| GREEN | **oodev** | 최소 구현 | 테스트 통과 |
| REFACTOR | **oodev** | 품질 개선 | lint 통과 |
| VERIFY | **ootest** | 런타임 검증 + 반복 루프 | import 테스트 통과 |
| COMPLETE | **oodev** | 문서 갱신 | d{SP}0003 [x] |
...
`oodev run dXXXX` 형식으로 특정 상세기획 문서를 지정하면, `d{SP}0002_plan.md` 대신 해당 문서를 개발 스펙으로 사용합니다.

```
oodev run d41001          # d41001_*.md 문서 기반 전체 개발
oodev run d41001 F001-1   # 특정 Feature만 구현
```
...
**예시**: `oodev run d41001` → `00_doc/sp04/d41001_데이터수집소스.md` 자동 탐색
...
```bash
oodev check --fix              # 전체
oodev check --fix [파일]       # 특정 파일
oodev check --fix --dry-run    # 분석만
```
