...
> ooreport와 구분: oopaper = **수집/정리** (외부 → 내부), ooreport = **출력/생성** (내부 → 외부)
...
```bash
oopaper doctor --check folder-id   # FOLDER_ID 위반 일괄 검증 (§13)
```
...
| 상황 | 조치 |
|------|------|
| 한 폴더에 PDF 2개 이상 | `oopaper split-multi --folder ID` 실행 → 첫 PDF는 그대로, 나머지는 **신규 FOLDER_ID 발급**하여 분리 |
...
4. 이동 후 `oopaper update` 호출하여 paper_list 동기화
...
| 5 | `oopaper update` 호출하여 paper_list 동기화 |

```bash
oopaper fix-suffix --batch --dry-run     # 모든 suffix 폴더 일괄 (계획만)
oopaper fix-suffix --batch                # 적용
oopaper fix-suffix --folder 260222-1851-08  # 특정 폴더만
oopaper fix-suffix --batch --use-mtime    # baseline 대신 mtime 기반 (예외)
```
...
| 명령어 | EN | KO | 설명 |
|--------|:--:|:--:|------|
| `oopaper help` | O | O | 서브명령어 목록 |
| `oopaper version` | O | O | 스킬 버전 (v08) |
| `oopaper status` | O | O | 폴더 현황·상태 통계 |
| `oopaper check` | O | O | `references/checklist.md` 기반 자체 점검 |
| `oopaper run [--lang]` | O | O | **통합 자동 파이프라인** (§7~§8) |
| `oopaper update [--lang] [--dry-run]` | O | O | **현행화** — paper_list 양방향 sync (§9) |
| `oopaper run this` | O | O | 직전 논문 계속 처리 |
| `oopaper anal [--deep] [--folder]` | O | O | 분석 — 서베이/정밀 (§10) |
| `oopaper trans [english\|korean]` | O | O | 텍스트 추출/번역 (§11) |
| `oopaper compress` | O | O | PDF 압축 (텍스트 보존) |
| `oopaper search` | O | - | 논문 검색 (arXiv, S2) |
| `oopaper download [--dry-run]` | O | - | 리스트 기반 PDF 자동 다운로드 |
| `oopaper extend [--folder] [--limit]` | O | - | 서머리 외부 참고문헌 → 자동 다운로드 (§12) |
| `oopaper ref` | O | - | 인용 검증·정규화 |
| `oopaper net` | O | - | 인용 네트워크 분석 |
| `oopaper keyword-sync` | O | - | 키워드 동기화 (서머리 → paper_list) |
| `oopaper ref-match` | O | - | 참고문헌 내부 매칭 |
| `oopaper fix [--folder] [--auto-fix]` | O | O | **정합성 검사·수정** (§13) — EN/KO 통합 |
| `oopaper doctor [--check NAME]` | O | O | **종합 진단** — folder-id, file-name, multi-pdf, sync, orphan (§13) |
| `oopaper split-multi --folder ID` | O | O | **다중 PDF 분리** — 신규 FOLDER_ID 발급 (§4) |
| `oopaper fix-suffix [--folder\|--batch]` | O | O | **suffix 폴더(`-NN`) 정규화** — baseline prefix 보존 rename (§4.3) |
| `oopaper backup [--dry-run]` | O | O | PDF 외부 경로 백업 |
| `oopaper restore [--dry-run]` | O | O | 백업 PDF 복원 |
| **자동화 (oopaper_auto.py)** | | | |
...
| --lang | 진입 스크립트 |
|--------|--------------|
| `en` (기본) | `uv run python .claude/skills/oopaper/scripts/oopaper_run.py [subcommand]` |
| `ko` | `uv run python .claude/skills/oopaper/scripts/ooessay_run.py [subcommand]` |
...
> Phase 7 및 `oopaper update`는 **양방향 검증**을 수행한다.
...
```bash
oopaper update --lang en --dry-run    # 변경 예정 확인
...
처리 흐름: 서머리 파싱 → 중복 제거 → arXiv/S2 검색 → `00_down/` 저장 → **`oopaper update` 수동 실행**
