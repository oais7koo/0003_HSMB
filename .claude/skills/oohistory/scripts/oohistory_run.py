...
def print_usage():
    ...
    print("oohistory - 프로젝트 변경 이력 관리")
    print()
    print("사용법:")
    print("    oohistory run              이력 현황 조회 (기본)")
    print("    oohistory run [태그] [제목] 이력 항목 추가")
    print("    oohistory list             최근 이력 조회 (최근 10건)")
    print("    oohistory create           d0010_history.md 신규 생성")
    print("    oohistory search [키워드]  이력 검색")
    print("    oohistory version          버전 목록 조회")
    print("    oohistory sync             oofix 완료 항목 동기화")
    print()
    print("태그 종류:")
    print("    HOTFIX   - 긴급 수정")
    print("    BUGFIX   - 버그 수정")
    ...
    print()
    print("예시:")
    print("    python .claude/skills/oohistory/scripts/oohistory_run.py run")
    print("    python .claude/skills/oohistory/scripts/oohistory_run.py run BUGFIX \"API 버그 수정\"")
    print("    python .claude/skills/oohistory/scripts/oohistory_run.py search DB")
    print("    python .claude/skills/oohistory/scripts/oohistory_run.py sync --dry-run")
...
def cmd_run(args):
    ...
    if not HISTORY_FILE.exists():
        ...
        print("[TIP] oohistory create 로 생성하세요.")
        return 0
...
def cmd_search(args):
    """이력 검색 (search 서브명령어)"""
    print("# oohistory search\n")

    if not args:
        print("[ERROR] 검색 키워드를 지정하세요.")
        print("사용법: oohistory search [키워드]")
        return 1
...
def cmd_sync():
    ...
    if dry_run:
        ...
        print("[TIP] 실제 동기화: oohistory sync (--dry-run 제거)")
        return 0
...
def main():
    ...
    if cmd == "run":
        ...
        print(f"[check] oohistory 체크리스트 안내")
