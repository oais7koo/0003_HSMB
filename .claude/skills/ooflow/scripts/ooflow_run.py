...
def cmd_status(sp_arg=None):
    ...
    s = scan_status(sp)
    print(f"[ooflow status] SP{sp:02d}")
    print()
    if not s['features']:
    ...
    print()
    if s['unstarted']:
        print(f"  미착수 {len(s['unstarted'])}개 → `ooflow run`으로 자동 처리 가능")
...
def cmd_check(sp_arg=None):
    ...
    if SKILL_MD.exists():
        ...
        tbl_m = re.search(r'ooflow version.*?\((v\d+)\)', text)
...
def build_plan(sp: int, until: Optional[str], feature_id: Optional[str]) -> dict:
    ...
    print(f"[ooflow dry-run] SP{sp:02d}")
    print()
    print("실행 예정 단계:")
