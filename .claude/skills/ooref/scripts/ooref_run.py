...
- ooref run [framework]: 특정 프레임워크 강제 지정

Usage:
    uv run python .claude/skills/ooref/scripts/ooref_run.py list
    uv run python .claude/skills/ooref/scripts/ooref_run.py run
    uv run python .claude/skills/ooref/scripts/ooref_run.py run D:/resilio/3_code/0002_CCone
    uv run python .claude/skills/ooref/scripts/ooref_run.py run fast-api
"""
...
def cmd_list():
    ...
    print("# ooref list - 프레임워크 레퍼런스 목록\n")
...
def cmd_run(args: list[str]):
    ...
    if not framework:
        ...
        print("- 또는 `ooref run [framework]` 으로 직접 지정")
        print("\n`ooref list` 로 지원 목록 확인")
        return
...
def cmd_add_checklist(args: list[str]):
    ...
    if not args:
        ...
        print("사용법: ooref add checklist \"항목 내용\"")
        return

    text = " ".join(args).strip().strip('"').strip("'")
    checklist_path = _SKILLS_DIR / "ooref" / "references" / "checklist.md"
    ...
    print(f"[ooref] 체크리스트 항목 추가: {text}")
...
def cmd_check():
    """checklist.md 기반으로 ooref 스킬 자체 기능 검증"""
    checklist_path = _SKILLS_DIR / "ooref" / "references" / "checklist.md"
    print("# ooref check — 스킬 기능 검증\n")
    ...
    check_item(
        "ooref run [path] — 특정 프로젝트 경로 지정 동작",
    ...
    _orig_path = _SKILLS_DIR / "ooref" / "references" / "checklist.md"
    ...
    try:
        ...
        check_item(
            "ooref add checklist — 항목이 checklist.md에 정상 추가되는지 확인",
            "- [ ] 테스트 항목" in _added,
        )
    finally:
    ...
    if not failed:
        ...
        print(f"[ACTION] {len(failed)}개 항목 수정 필요 — `ooref` 스크립트 검토")
...
def main():
    ...
    if show_help_if_no_args("ooref", args):
        return
