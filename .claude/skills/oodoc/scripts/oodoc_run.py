...
사용법:
    uv run .claude/skills/oodoc/scripts/oodoc_run.py           # 현재 컨텍스트 SP 문서 일괄 실행
    uv run .claude/skills/oodoc/scripts/oodoc_run.py --all     # 전체 SP 일괄 실행
    uv run .claude/skills/oodoc/scripts/oodoc_run.py --dry-run # 드라이런
    uv run .claude/skills/oodoc/scripts/oodoc_run.py --required-only # 필수만
    uv run .claude/skills/oodoc/scripts/oodoc_run.py --doc d0004_todo.md # 특정 문서만
"""
...
DOC_SKILL_MAPPING = [
    ...
    ("d0008_checklist.md", "oocheck", "oocheck update", False),
...
def cmd_list_all():
    ...
    if all_missing:
        ...
        cmds = "  |  ".join(f"`oodoc gen --sp {sp}`" for sp in sps)
        print(f"[GEN] {cmds}")
    else:
...
def cmd_gen(sp_num: int, dry_run: bool = False):
    ...
    sp_label = f"SP{sp_num:02d}"
    sp_doc_dir = get_sp_doc_dir(sp_num)
    print(f"# oodoc gen - {sp_label} 빈 문서 생성\n")
    if dry_run:
    ...
    if dry_run:
        print(f"\n[TIP] 실제 생성: `oodoc gen --sp {sp_num}` (--dry-run 제거)")
...
# 값: .claude/skills/ 기준 상대 경로
# 그 외(ooprd/ooplan/oolib/oodb 등)는 문서 내용 생성에 LLM 추론이 필요 → 계획만 출력
SCRIPT_EXECUTABLE = {
    "oohistory sync": "oohistory/scripts/oohistory_run.py",
}
...
def execute_skill(skill_command: str, dry_run: bool = False, sp_num: int = 0) -> tuple[bool, str]:
    ...
    # 스크립트 직접 실행 대상
    script_path = PROJECT_ROOT / ".claude" / "skills" / script_rel
    if not script_path.exists():
        return False, f"스크립트 없음: {script_rel}"

    sub_args = skill_command.split()[1:]  # "oohistory sync" -> ["sync"]
...
def cmd_update(sp_num: int, scope: str = "auto", commit: str = "HEAD~1", dry_run: bool = False) -> int:
    ...
    print(f"# oodoc update — SP{sp_num:02d}\n")
    ...
    print(f"- 문서 생성/누락 보충: `oodoc gen --sp {sp_num}`")
    print(f"- 일괄 실행: `oodoc run --sp {sp_num}`")
...
def generate_doc_list() -> Path:
    ...
    lines = [
        "# 00_doc 문서 목록",
        "",
        "> oodoc run/update가 자동 생성. 각 00_doc/sp{N}/ 폴더의 최상위 .md 문서를 위키 링크로 정리한다.",
        "",
        "## 문서 이력 관리",
        f"- {now} — oodoc 자동 생성/업데이트",
        "",
    ]
    total = 0
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="oodoc run - 문서 일괄 업데이트 도구"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if args.command == "list":
        print(f"# oodoc list - SP{sp_num:02d} 문서 현황\n")
        ...
        if missing:
            ...
            print()
            print(f"[GEN] `oodoc gen --sp {sp_num}`")
        else:
            print("[OK] 모든 필수 문서가 존재합니다.")
        return 0
    ...
    if args.all:
        ...
        print(f"[oodoc run --all] 전체 SP 처리: {labels}\n")
        total_fail = 0
        for n in all_sp:
            print("=" * 50)
            print(f"## SP{n:02d}")
