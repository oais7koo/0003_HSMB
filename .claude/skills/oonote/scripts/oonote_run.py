...
사용법:
    uv run python .claude/skills/oonote/scripts/oonote_run.py add "내용" [--title "제목"] [--tag "태그"] [--ref "참조"] [--sp N]
    uv run python .claude/skills/oonote/scripts/oonote_run.py today [--sp N]
    uv run python .claude/skills/oonote/scripts/oonote_run.py list [날짜] [--all] [--sp N]
    uv run python .claude/skills/oonote/scripts/oonote_run.py search "키워드" [--tag "태그"] [--date "YYYY-MM"] [--limit N] [--sp N]
    uv run python .claude/skills/oonote/scripts/oonote_run.py status [--sp N]
    uv run python .claude/skills/oonote/scripts/oonote_run.py init [--sp N]
"""
...
# scripts/ → oonote/ → skills/ → .claude/ → PROJECT_ROOT
...
def cmd_status(args):
    ...
    print(f"[oonote status] {sp_label}")
    print(f"파일  : {note_file.relative_to(PROJECT_ROOT)}")

    if not note_file.exists():
        print("상태  : 파일 없음 (oonote init 으로 생성)")
        return
...
def main():
    if len(sys.argv) > 2 and sys.argv[1].lower() == "show" and sys.argv[2].lower() == "checklist":
        cmd_show_checklist()
        return
    if show_help_if_no_args("oonote", sys.argv[1:]):
        return 0
    parser = argparse.ArgumentParser(description="oonote - 연구노트 관리")
