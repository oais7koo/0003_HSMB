---
name: oonote
description: "연구노트 작성 스킬 'oonote', '연구노트', '노트 추가', '연구 기록', '실험 기록' 등을 요청할 때 트리거된다"
metadata:
  version: "v01"
  category: "doc-env"
---
...
**핵심 기능**:
1. **노트 추가**: `oonote add "내용"` → 오늘 날짜 섹션에 자동 기록
2. **오늘 조회**: `oonote today` → 오늘 작성된 노트 표시
3. **날짜 조회**: `oonote list [날짜]` → 특정 날짜 노트 표시
4. **검색**: `oonote search "키워드"` → 전체 노트 검색
5. **현황**: `oonote status` → 통계 및 최근 항목
...
| 옵션 | 설명 | 예시 |
|------|------|------|
| `--title "제목"` | 제목 지정 | `oonote add --title "실험 결과" "내용"` |
| `--tag "태그"` | 태그 지정 | `oonote add --tag "실험" "내용"` |
| `--ref "참조"` | 참조 추가 | `oonote add --ref "d0002_plan.md" "내용"` |
| `--time "HH:MM"` | 시간 수동 지정 | `oonote add --time "09:00" "내용"` |
| `--sp N` | 서브프로젝트 지정 | `oonote add --sp 03 "내용"` |
