...
def print_usage():
    ...
    print("ooreport - 자동 리포트 생성")
    print()
    print("사용법:")
    print("    ooreport run              신규 리포트 생성 (기본)")
    print("    ooreport update [name]    기존 리포트 업데이트")
    print("    ooreport list             생성된 리포트 목록 확인")
    print()
    print("옵션:")
    ...
    print()
    print("예시:")
    print("    python .claude/skills/ooreport/scripts/ooreport_run.py run --template weekly")
    print("    python .claude/skills/ooreport/scripts/ooreport_run.py update weekly_report_2025_w01.md")
    print("    python .claude/skills/ooreport/scripts/ooreport_run.py list")
...
def cmd_update(args, options):
    ...
    dry_run = options.get("dry_run", False)
    print("# ooreport update\n")
    if dry_run:
        print("[DRY-RUN] 실제 파일 수정 없이 미리보기만 실행\n")

    if not args:
        print("[ERROR] 리포트 파일명을 지정하세요.")
        print("사용법: ooreport update [name]")
        print()
        print("[TIP] ooreport list 로 목록을 확인하세요.")
        return 1
...
def cmd_list():
    ...
    if not REPORTS_DIR.exists():
        ...
        print("[TIP] ooreport run 으로 리포트를 생성하세요.")
        return 0
    ...
    print()
    print("[TIP] ooreport update [name] 으로 업데이트할 수 있습니다.")
...
def main():
    ...
    if cmd == "run":
        ...
        print(f"[check] ooreport 체크리스트 안내")
