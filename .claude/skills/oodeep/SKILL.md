---
name: oodeep
description: "딥러닝 GPU 효율성 최적화 스킬 'oodeep', '딥러닝', 'GPU 최적화', '학습 실행', 'train' 등의 키워드로 트리거된다"
metadata:
  version: v01
  category: core-dev
---
...
| 명령어 | 설명 |
|--------|------|
| `oodeep help` | 서브명령어 목록 표시 |
| `oodeep version` | 스킬 버전 정보 (v01) |
| `oodeep status` | 서브명령어 리스트, GPU 현황 |
| `oodeep check` | references/checklist.md 기반 검증 |
| `oodeep check [script]` | references/checklist.md 기반 체크 및 리포팅 / 스크립트 GPU 효율성 사전 분석 (실행 없음) | 터미널 |
| `oodeep show checklist` | 역할 수행 체크리스트 표시 |
| `oodeep add checklist "항목"` | 체크리스트 항목 추가 |
| `oodeep run [script]` | 딥러닝 스크립트 실행 + GPU 최적화 사이클 |
| `oodeep run [script] --max-iter N` | 최대 반복 횟수 지정 (기본: 5) |
| `oodeep run [script] --interval N` | 모니터링 간격 초 (기본: 30) |
| `oodeep run [script] --threshold N` | GPU 사용률 임계값 % (기본: 50) |
| `oodeep monitor` | GPU 상태만 모니터링 (수정 없음) |
...
실행: `uv run python .claude/skills/oodeep/scripts/oodeep_monitor.py`
...
- `.claude/skills/oodeep/references/guide.md`: 상세 최적화 가이드
