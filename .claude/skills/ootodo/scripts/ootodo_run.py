...
Usage:
    ootodo "내용"          # 추가 + 즉시 처리
    ootodo                 # 대기 중 전체 처리
    ootodo list            # 전체 SP 대기 업무 조회
    ootodo status          # 상태 조회
    ootodo add [text]      # 추가만 (처리 안함)
    ootodo clear           # 완료된 Todo → d0010 아카이브
"""
import sys
import re
...
def add_single_to_history(history_content: str, todo: dict, sp: str = "00") -> str:
    ...
    new_entry = (
        ...
        f"- __작업 유형__: CUSTOM (ootodo audit)\n"
        f"{done_body_section}\n"
        f"---\n\n"
    )
...
def add_to_history_archive(content: str, issues: list[dict], sp: str = "00") -> str:
    ...
    new_entry = (
        f"## {today}\n\n"
        f"### {next_num}. [ARCHIVE] ootodo clear — SP{sp} 완료 항목 아카이브\n\n"
        f"#### {next_num}.1. 작업 내용\n\n"
        f"- __상태__: [DONE]\n"
        f"- __완료일__: {today}\n"
        f"- __작업 유형__: ARCHIVE (ootodo clear)\n\n"
...
def cmd_status(sp: str = "00"):
    ...
    print("# ootodo status - 대기 중인 Todo 목록\n")
    if not todos:
        print("(대기 중인 항목 없음)")
        return
...
def cmd_audit(sp: str = "00"):
    ...
    for i, m in enumerate(splits, 1):
        ...
        print(f"\n**판정 후 실행**:")
        print(f"```")
        print(f"# OK  → ootodo history {todo_id} --sp {sp}")
        print(f"# FAIL→ ootodo reopen {todo_id} \"실패 이유\" --sp {sp}")
        print(f"```\n")
        print(f"---\n")
...
def cmd_update(sp: str = "00", message: str | None = None, no_sort: bool = False, dry_run: bool = False):
    ...
    print(f"# ootodo update (SP{int(sp):02d})\n")
    ...
    if completed:
        print(f"\n→ 완료 항목 검증: `ootodo audit --sp {sp}`")
        print(f"→ 무효 항목 복원: `ootodo reopen [ID] \"메모\" --sp {sp}`")
        print(f"→ 검증 후 아카이브: `ootodo history [ID] --sp {sp}` 또는 `ootodo clear --sp {sp}`")
    print()
    ...
    msg = message or "todo 문서 현행화 (ootodo update)"
...
def main():
    ...
    if subcommand == "reopen":
        if len(args) < 2:
            print("사용법: ootodo reopen [ID] [\"메모\"]  (예: ootodo reopen C012 \"미구현\")")
            return
    ...
    if subcommand == "history":
        if len(args) < 2:
            print("사용법: ootodo history [ID]  (예: ootodo history C012)")
            return
        todo_id = args[1].upper()
    ...
    if subcommand == "complete":
        if len(args) < 2:
            print("사용법: ootodo complete [ID]  (예: ootodo complete C012)")
            return
        todo_id = args[1].upper()
        sp_val = sp
    ...
    if first_arg and not first_arg.startswith("-"):
        ...
        if single_word and first_arg.lower() not in KNOWN_SUBS:
            ...
            print("        ToDo로 추가하려면 따옴표로 감싸세요: ootodo \"내용\"")
            print("        서브커맨드 목록: ootodo help")
            return
    ...
    print("ootodo - Todo 자동 처리 스킬")
    print("\n사용법:")
    print('  ootodo "내용"         # 추가 + 즉시 처리')
    print("  ootodo                # 대기 중 업무 전체 처리")
    print("  ootodo list           # 전체 SP 대기 업무 표시")
    print("  ootodo status         # 현재 SP 대기 중 목록 표시")
    print('  ootodo add [text]     # 추가만 (처리 안함)')
    print("  ootodo clear          # 완료 항목 → d0010_history.md 아카이브")
    print("\n옵션:")
