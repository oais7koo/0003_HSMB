---
name: ooskill
description: "스킬 최적화 검증 스킬 'ooskill', '스킬 최적화', '스킬 검증', '에이전트 최적화' 등을 요청할 때 트리거된다"
metadata:
  version: "v04"
  category: "meta-util"
---
...
| `ooskill status` | 서브명령어 목록, 스킬 현황, 최적화 요약 | - |
| `ooskill check` | 현재 상황 검토 후 최적안 제안 (구 `validate`) | 개선 권장사항 |
| `ooskill check --checklist` | 모든 oo* 스킬 checklist.md 표준 포맷 검증 (구 `validate-checklist`) | 터미널 |
| `ooskill show checklist` | 역할 수행 체크리스트 표시 | 터미널 |
| `ooskill add checklist "항목"` | 체크리스트 항목 추가 | checklist.md |
| `ooskill run` | 스킬 파일 배치 실행 (SKILL.md 자동 수정) | 수정된 스킬 파일 |
| `ooskill run --dry-run` | 변경 예정 내용 미리 출력 (실제 수정 안 함) | 터미널 |
| `ooskill run [스킬명]` | 특정 스킬만 배치 실행 | 수정된 스킬 파일 |
| `ooskill update` | 등록된 모든 현행화 타겟 일괄 실행 (catalog + run-update-ref + gemma) | 수정된 파일 |
| `ooskill update <타겟>` | 특정 타겟만 현행화 (예: `ooskill update catalog`) | 수정된 파일 |
| `ooskill update <타겟> --dry-run` | 변경 예정 내용만 출력 | 터미널 |
| `ooskill backup` | ~~제거~~ → `oosync backup` 사용 | - |
...
> 예시: `ooskill run`은 스킬 파일 자동 수정(배치), `ooskill update`는 CLAUDE.md 카탈로그 + Gemma 참조 등 현행화 전용.
...
| 항목 | 기준 | 예시 |
|------|------|------|
| 백틱 | 명령어 전체를 백틱으로 감쌀 것 | `` `ooskill version` `` |
| 버전 번호 | `(vXX)` 형식으로 포함 | `(v01)`, `(v12)` |
| 위치 | 서브명령어 테이블에서 `help` 행 바로 다음 | 두 번째 행 |
| 출력 | 스킬 이름 + 버전 번호 표시 | `ooskill v01` |
