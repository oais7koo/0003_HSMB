"""
[oobook_backup.py]
03_paper/ 내 도서 PDF 파일을 외부 백업 경로로 이동하거나 복원합니다.
hostname을 자동 감지하여 컴퓨터별 백업 경로를 선택합니다.
backup_config.json은 oopaper와 공유 (같은 백업 위치).

사용법:
    uv run python .claude/skills/oobook/scripts/oobook_backup.py backup [--dry-run] [--code CODE] [--subdir DIR]
    uv run python .claude/skills/oobook/scripts/oobook_backup.py restore [--dry-run] [--code CODE] [--subdir DIR]
    uv run python .claude/skills/oobook/scripts/oobook_backup.py status
    uv run python .claude/skills/oobook/scripts/oobook_backup.py config --path PATH
...
# scripts/ → oobook/ → skills/ → .claude/ → PROJECT_ROOT
...
# 도서 백업 대상 하위 폴더
BOOK_SUBDIRS = ["01_book_ko", "02_book_en", "03_book_ja"]
...
def collect_pdfs(base_dir: Path, subdirs: list, code_filter: str = None, subdir_filter: str = None):
    """base_dir 내 도서 PDF 파일 목록 수집.
    code_filter: 도서코드 앞부분 매칭 (예: BK0200, BE5400)
    """
    results = []
    ...
    for subdir in targets:
        ...
        for pdf in sorted(src.rglob("*.pdf")):
            ...
            if code_filter:
                # 도서 폴더명이 code_filter로 시작하는지 확인
                parts = rel.parts
                if len(parts) < 2 or not parts[1].startswith(code_filter):
                    continue
...
def cmd_backup(args):
    ...
    pdfs = collect_pdfs(PAPER_BASE, BOOK_SUBDIRS, args.code, args.subdir)
    if not pdfs:
        print("[INFO] 백업할 도서 PDF 없음.")
        return
...
def cmd_restore(args):
    ...
    pdfs = collect_pdfs(backup_base, BOOK_SUBDIRS, args.code, args.subdir)
    if not pdfs:
        print("[INFO] 복원할 도서 PDF 없음.")
        return
...
def main():
    parser = argparse.ArgumentParser(description="oobook backup/restore (hostname 자동 감지)")
    sub = parser.add_subparsers(dest="command")

    # backup
    p_bak = sub.add_parser("backup", help="도서 PDF를 백업 경로로 이동")
    p_bak.add_argument("--dry-run", action="store_true", help="실제 이동 없이 목록만 출력")
    p_bak.add_argument("--code", metavar="CODE", help="특정 도서코드 앞부분만 (예: BK0200, BE5400)")
    p_bak.add_argument("--subdir", metavar="DIR", help="특정 하위 폴더만 (예: 01_book_ko)")

    # restore
    p_res = sub.add_parser("restore", help="백업 경로에서 도서 PDF 복원")
    p_res.add_argument("--dry-run", action="store_true", help="실제 이동 없이 목록만 출력")
    p_res.add_argument("--code", metavar="CODE", help="특정 도서코드 앞부분만")
    p_res.add_argument("--subdir", metavar="DIR", help="특정 하위 폴더만")

    # status
    sub.add_parser("status", help="원본/백업 도서 PDF 현황")
