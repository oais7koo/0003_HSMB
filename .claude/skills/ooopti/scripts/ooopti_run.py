"""ooopti - 알고리즘/코드 최적화 스크립트

대상 파일의 성능 프로파일링, 복잡도 분석, 벤치마크를 수행한다.

Usage:
    uv run python .claude/skills/ooopti/scripts/ooopti_run.py profile <file>
    uv run python .claude/skills/ooopti/scripts/ooopti_run.py complexity <file>
    uv run python .claude/skills/ooopti/scripts/ooopti_run.py benchmark <file> <function>
    uv run python .claude/skills/ooopti/scripts/ooopti_run.py status
"""
...
def cmd_complexity(target: str):
    ...
    print(f"[ooopti complexity] {target}")
    print()
...
def cmd_benchmark(target: str, func_name: str, iterations: int = 1000):
    """함수 벤치마크."""
    print(f"[ooopti benchmark] {target}::{func_name}")
...
def main():
    ...
    if show_help_if_no_args("ooopti", args):
        return
