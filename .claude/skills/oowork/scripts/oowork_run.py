...
def status(process_name: str, workdir: str = "."):
    """프로세스 진행 상태 확인"""
    state_dir = Path(workdir) / ".oowork"
    state_file = state_dir / f"{process_name}_state.json"

    print(f"# oowork status: {process_name}\n")
...
def _resolve_output_path(input_path: Path, output_arg: str | None) -> Path:
    """출력 경로 결정: 명시된 경우 그대로, 없으면 oowork 넘버링 규칙 적용.
...
def extract(input_path_str: str, output_path_str: str | None = None) -> None:
    ...
    if ext in HWP_EXTS:
        ...
        print("  이후 oowork extract <변환된.pdf> 를 실행하세요.")
...
def main():
    ...
    if show_help_if_no_args("oowork", args):
        return
