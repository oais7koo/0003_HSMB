---
name: ooresearch
description: "SOTA 기반 연구 수행 스킬 'ooresearch', '연구 수행', 'SOTA 분석', '최신 연구 동향', '연구 조사', 'sota 비교' 등을 요청할 때 트리거된다"
metadata:
  version: "v02"
  category: "content"
---
...
| 스킬 | 역할 | 순서 |
|------|------|------|
| **`ooresearch`** | **논문 작성 전 조사** (SOTA 파악, 관련 연구 수집, 갭 분석) | 1단계 |
| `oosota` | 논문 초안 작성 (ooresearch 결과를 입력으로 활용) | 2단계 |

→ `ooresearch` 산출물(`d{SP}0200_research_*.md`)이 `oosota`의 related 섹션 입력이 됨.
...
| 명령어 | 설명 |
|--------|------|
| `ooresearch help` | 서브명령어 목록 표시 |
| `ooresearch version` | 스킬 버전 정보 (v02) |
| `ooresearch status` | 현재 연구 문서 목록, 보유 논문 현황, 도메인 현황 |
| `ooresearch check` | references/checklist.md 기반 체크 및 리포팅 |
| `ooresearch run` | 연구 수행 실행 |
| `ooresearch show checklist` | 역할 수행 체크리스트 표시 |
| `ooresearch add checklist "항목"` | 체크리스트 항목 추가 |
| `ooresearch run [topic]` | 종합 연구 수행 → 연구 문서 생성 |
| **`ooresearch run this`** | **직전 연구 주제 계속** (→ common_guide.md §9) |
| `ooresearch sota [topic]` | SOTA 현황만 조사 (빠른 스냅샷) |
| `ooresearch compare [topic]` | 주요 접근법 SOTA 비교 분석 |
| `ooresearch trend [topic]` | 연구 트렌드 및 방향성 분석 |
| `ooresearch gap [topic]` | SOTA 대비 연구 갭 분석 |
...
| `.claude/skills/oosota/SKILL.md` | **다음 단계**: ooresearch 결과 → 논문 초안 작성 |
| `00_doc/d{SP}0200_research_*.md` | 연구 결과 문서 (oosota 입력) |
| `.claude/skills/ooresearch/references/guide.md` | 도메인별 상세 가이드 |
