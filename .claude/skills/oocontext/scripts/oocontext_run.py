...
서브프로젝트 컨텍스트 관리 (.claude/skills/oocontext/SKILL.md 구현)
...
사용법:
    python .claude/skills/oocontext/scripts/oocontext_run.py
    python .claude/skills/oocontext/scripts/oocontext_run.py 04
    python .claude/skills/oocontext/scripts/oocontext_run.py clear
    python .claude/skills/oocontext/scripts/oocontext_run.py list
"""
...
def cmd_set(sp: str, dry_run: bool = False):
    ...
    if dry_run:
        print(f"## oocontext - SP{sp} 설정 예정 (dry-run)\n")
...
def cmd_clear():
    save_context("00")
    print("## oocontext - 공통(SP00)으로 초기화\n")
...
def cmd_list():
    ...
    print(f"## oocontext - SP 목록 ({config_src})\n")
...
def cmd_update(dry_run: bool = False):
    ...
    print(f"[oocontext sp_config 갱신{'  --dry-run' if dry_run else ''}]\n")
...
def cmd_status():
    print("## oocontext v09 - 서브프로젝트 컨텍스트 관리\n")
    print("### 서브명령어\n")
    print("| 명령어 | 설명 |")
    print("|--------|------|")
    print("| `oocontext` | 현재 컨텍스트 확인 |")
    print("| `oocontext [N]` | SP N으로 설정 (sp_config.json 자동 갱신 포함) |")
    print("| `oocontext clear` | 공통(00) 초기화 |")
    print("| `oocontext list` | SP 목록 표시 |")
    print("| `oocontext status` | 현재 상태 (이 화면) |")
    print("| `oocontext version` | 버전 정보 |")
    print()
    cmd_show()
...
def main():
    ...
    if cmd in ("help", "-h", "--help"):
        ...
        print(f"[check] oocontext 체크리스트 안내")
        _print_skill_help("oocontext")
    elif cmd == "update":
        ...
        print("사용법: oocontext [N | clear | list | status | version]")
        sys.exit(1)
