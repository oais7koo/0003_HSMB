#!/usr/bin/env python3
"""oobatch - 모든 oo* 스킬에 동일한 서브명령어 일괄 실행"""
...
def find_skills_root() -> Path:
    # .claude/skills/oobatch/scripts/oobatch_run.py → .claude/skills/
...
def scan_skills_for_subcmd(skills_root: Path, subcmd: str) -> list[dict]:
    ...
    for skill_dir in sorted(skills_root.glob("oo*")):
        if not skill_dir.is_dir() or skill_dir.name == "oobatch":
            continue
