...
명령어:
    ooprd run              새 PRD 문서 생성 (기본)
    ooprd run [번호]       서브프로젝트 PRD 생성
    ooprd template         PRD 템플릿 구조 조회
    ooprd validate         PRD 구조 검증 및 누락 항목 체크
    ooprd update           PRD 현행화 (실제 프로젝트와 동기화)
    ooprd section [번호]   특정 섹션만 생성/갱신
"""
...
def print_usage():
    ...
    print("ooprd - PRD 문서 생성 및 관리")
    print()
    print("사용법:")
    print("    ooprd run              새 PRD 문서 생성 (기본)")
    print("    ooprd run [번호]       서브프로젝트 PRD 생성")
    print("    ooprd template         PRD 템플릿 구조 조회")
    print("    ooprd validate         PRD 구조 검증 및 누락 항목 체크")
    print("    ooprd update           PRD 현행화")
    print("    ooprd section [번호]   특정 섹션만 생성/갱신")
    print()
    print("예시:")
    print("    python .claude/skills/ooprd/scripts/ooprd_run.py run")
    print("    python .claude/skills/ooprd/scripts/ooprd_run.py validate")
    print("    python .claude/skills/ooprd/scripts/ooprd_run.py template")
...
def cmd_validate():
    ...
    print("# ooprd validate\n")

    if not PRD_FILE.exists():
        print(f"[ERROR] {PRD_FILE}가 없습니다.")
        print("[TIP] ooprd run 으로 생성하세요.")
        return 1
...
def cmd_section(args):
    ...
    print("[TIP] ooprd template 으로 템플릿 구조를 확인하세요.")
