...
TEMPLATE_DIR = SCRIPT_DIR.parent / "templates"  # .claude/skills/oosync/templates
...
# 템플릿 문자열 정의
TEMPLATE_VIEW_HEADER = """# oosync view - 차이점 비교
...
TEMPLATE_DIFF_ALL_HEADER = """# oosync diff - 전체 차이점 비교
...
def cmd_status():
    ...
    print("# oosync - Vibe 환경 동기화\n")
    print("## 서브명령어\n")
    print("| 명령어 | 설명 |")
    print("|--------|------|")
    print("| `oosync status` | 현재 상태 (이 화면) |")
    print("| `oosync list` | 동기화 가능한 프로젝트 목록 |")
    print("| `oosync files` | 동기화 대상 파일/폴더 목록 |")
    print("| `oosync view [project]` | 차이점 비교 (읽기 전용) |")
    print("| `oosync run [project]` | 동기화 실행 |")
    print("| `oosync run --push-only` | **push만 필요한 모든 프로젝트 일괄 동기화** |")
    print("| `oosync run --push-only --dry-run` | 동기화 미리보기 (실제 복사 안 함) |")
    print("| `oosync diff [project] [file]` | 파일 내용 비교 |")
    print()
    print(f"## 현재 프로젝트\n")
...
def cmd_view(project_name: str | None = None):
    ...
    if not project_name:
        print("# oosync view - 차이점 비교\n")
        print("사용법: `oosync view [project_name]`\n")
        print("프로젝트명 또는 4자리 번호 입력 가능 (예: 0003 또는 0003_CCone)\n")
        print("`oosync list`로 프로젝트 목록을 먼저 확인하세요.")
        return
...
def sync_project_files(target_project: Path, push_files: list, delete_files: list = None, dry_run: bool = False) -> bool:
    ...
    if not dry_run:
        new_hashes = {}
        for rel_path in push_files:
            tp = target_project / rel_path
            if tp.exists() and tp.is_file():
                new_hashes[str(rel_path).replace("\\", "/")] = get_file_hash(tp)
        existing_state = load_sync_state(project_name)
        merged_hashes = {**existing_state.get("file_hashes", {}), **new_hashes}
        save_sync_state(project_name, current_hash or "", merged_hashes)
        if current_hash:
            print(f"    [GIT] 동기화 상태 기록: {current_hash[:8]} ({len(new_hashes)}개 파일 해시 갱신)")
        else:
            print(f"    [HASH] 동기화 상태 기록: {len(new_hashes)}개 파일 해시 갱신")
...
def cmd_run_push_only(dry_run: bool = False, allow_delete: bool = False, allow_add: bool = False, force_conflict: bool = False):
    ...
    if all_conflicts:
        ...
        for proj, conflict_files in all_conflicts:
            ...
            print(f"merge 명령어: `oosync merge {proj.name} <파일경로>`")
            print(f"내용 비교:    `oosync diff {proj.name} <파일경로>`\n")
    ...
    if dry_run:
        ...
        print("\n[TIP] 실제 동기화: `oosync run --push-only` (--dry-run 제거)")
    else:
        print(f"\n## 완료\n")
        if sync_targets:
...
def cmd_run(project_name: str | None = None):
    ...
    print("# oosync run - 동기화 실행\n")

    if not project_name:
        print("사용법: `oosync run [project_name]`\n")
        print("프로젝트명 또는 4자리 번호 입력 가능 (예: 0003 또는 0003_CCone)\n")
        print("`oosync list`로 프로젝트 목록을 먼저 확인하세요.")
        return
...
def cmd_diff(project_name: str | None, filename: str | None):
    ...
    if not project_name:
        print("# oosync diff - 파일 비교\n")
        print("사용법: `oosync diff [project] [filename]`\n")
...
def cmd_merge(project_name: str | None, filename: str | None, direction: str = "auto"):
    ...
    if not project_name:
        print("# oosync merge - 파일 병합\n")
        print("사용법: `oosync merge [project] [filename]`\n")
        print("옵션:")
...
def cmd_reset(args: list):
    ...
    if not args or args == ["--show"]:
        ...
        print("\n사용법:")
        print("  oosync reset --targets /path1 /path2  # 대상 설정 (하나 이상)")
        print("  oosync reset --add /path              # 대상 추가")
        print("  oosync reset --remove /path           # 대상 제거")
        print("  oosync reset --clear                  # 대상 전체 제거")
        return
    ...
    if "--targets" in args:
        ...
        print("사용법: oosync reset [--targets /path1 /path2 | --add /path | --remove /path | --clear]")
...
def cmd_restore(backup_name=None):
    ...
    if backup_name:
        ...
        if len(backups) > 10:
            print(f"\n... 외 {len(backups) - 10}개")
        print(f"\n사용법: oosync restore <파일명 또는 번호>")
        print(f"예: oosync restore {backups[0].stem}")
        return

    # 복원 실행
    print(f"[oosync restore]")
...
def main():
    ...
    if subcommand == "status":
        ...
        if "--push-only" in sys.argv:
            ...
            cmd_run_push_only(dry_run=dry_run, allow_delete=allow_delete, allow_add=allow_add, force_conflict=force_conflict)
        else:
            project_name = sys.argv[2] if len(sys.argv) > 2 else None
        ...
        print("'oosync status'로 사용 가능한 명령어를 확인하세요.")
