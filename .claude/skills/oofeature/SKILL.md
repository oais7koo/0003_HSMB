...
| `oofeature help` / `version` / `status` | 도움말 / 버전 / 현재 SP 현황 |

| `oofeature new dXXXX "기능명" [--stage 단계]` | 상세 문서 생성 (`--stage` 미지정 시 기획) |

| `oofeature next dXXXX` / `next this` | 다음 단계 rename + 연계 스킬 실행 (`this`=직전 작업, common_guide §9) |

| `oofeature stage dXXXX 단계` | 단계 수동 변경 (rename만) |

| `oofeature list` | 현재 SP 상세 문서 목록 + 단계 현황 |

| `oofeature needed [--sp N]` | plan.md Feature ↔ 상세 문서 교차 비교 → 미착수 리스트업 |

| `oofeature sync` | plan.md 8.2절 강제 갱신 |

| `oofeature update <doc_id> [--sp N]` | **문서↔코드 비교 → 현행화 → 미구현 배치** (oocommit 연동) |
| `oofeature update` | 변경 감지 → 상세기획 롤백 후보 표시 |

| `oofeature update --apply` / `--force` | 롤백 실행 (`--force`=`--apply` 별칭) |

| `oofeature update --dry` | 롤백 미리보기 |

| `oofeature update --from-plan` / `--from-doc` | (하위 호환) plan.md / 상세구현+oocheck 기반 롤백 |

| `oofeature note dXXXX "내용" [--sp N]` | ## 메모 섹션에 날짜+내용 추가 |

| `oofeature issue dXXXX "이슈내용" [--sp N]` | ## 이슈 섹션에 추가 (🔴 미해결) |

| `oofeature issue dXXXX --resolve` | 최신 미해결 이슈 → ✅ 해결 |

| `oofeature check [--sp N] [--verbose] [--dry-run] [--checklist]` | 정합성 검증 (V01~V08) |

| `oofeature rmdup [--sp N] [--all] [--dry-run]` | 같은 번호 복수 단계 파일 → 최신만 유지 |
